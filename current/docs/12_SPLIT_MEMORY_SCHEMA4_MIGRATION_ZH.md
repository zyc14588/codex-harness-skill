...
schema 4 建立了基础设施与 task-shape 的归因边界，但旧 DSH managed minimal MCP 子进程启动失败时只输出通用 preset-mount 错误，旧分类器没有写入 `infrastructureFailureKind`。结果是零 Provider I/O、零 Token、空 diff 的启动失败仍可能被 schema 4 记成 task-shape anomaly。schema 5 隔离这批画像；0.6.6 已移除模型可见 MCP 子进程，并将 broker plugin 组合失败和 `no_effect` 归入不可学习的基础设施证据。
...
0.6.6 保留 schema-v5 的失败归因，但已经修改 Provider/Harness/tool 路径，因此 R2/R4 证据只作历史，不能继承为当前资格。当前 revision 的完整 regression、release bindings、真实 Flash/Pro 与最终解包归档必须重新执行；完成前权威状态是 candidate。
