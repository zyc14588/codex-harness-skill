...
args=sys.argv[1:]
if args in (["--version"],["-V"]): print("codex-fake "+os.environ["CODEX_HARNESS_PACKAGE_VERSION"]); raise SystemExit(0)
...
[[ -f "$MINIMAL_PROFILE_DIR/bridge-brokered-tools.mjs" ]]
[[ -f "$MINIMAL_PRESET_DIR/.codex-harness-bridge-managed.json" ]]
[[ -f "$MINIMAL_PRESET_DIR/agent.cordis.yml" ]]
python3 - "$MINIMAL_PRESET_DIR/agent.cordis.yml" "$INSTALL_ROOT/harness/minimal/preset/agent.cordis.yml.in" <<'PY_PRESET'
import sys
installed,template=sys.argv[1:]
expected=open(template,encoding='utf-8').read().replace(
    '{{CODEX_HARNESS_BROKER_PLUGIN}}',
    '/sandbox/dsh/profiles/codex-minimal-headless/bridge-brokered-tools.mjs',
)
assert open(installed,encoding='utf-8').read()==expected
PY_PRESET
grep -F "name: '/sandbox/dsh/profiles/codex-minimal-headless/bridge-brokered-tools.mjs'" "$MINIMAL_PRESET_DIR/agent.cordis.yml" >/dev/null
! grep -E "dsh-mcp-client|minimal-tools-server|tool-sandbox-entry|^[[:space:]]*(command|args):" "$MINIMAL_PRESET_DIR/agent.cordis.yml" >/dev/null
...
echo "[package 7/9] Reinstall accepted $VERSION after rollback"
...
! awk '{print $2}' "$SOURCE_ROOT/MANIFEST_SHA256.txt" | grep -F '/node_modules/' >/dev/null
[[ -f "$SOURCE_ROOT/bridge/package-lock.json" ]]
find "$SOURCE_ROOT" -path '*/node_modules' -prune -o -type l -print -quit | grep -q . \
  && { echo "Package contains a symlink outside the excluded dependency tree" >&2; exit 1; } || true
python3 - "$SOURCE_ROOT" "$VERSION" <<'PY'
import hashlib,json,os,sys
root,expected_version=sys.argv[1:]
for rel in [
...
for name in os.listdir(os.path.join(root,'evidence')):
    if name.endswith('.json'):
        json.load(open(os.path.join(root,'evidence',name),encoding='utf-8'))
...
for rel in [
  'docs/01_ARCHITECTURE_ZH.md','docs/02_INSTALL_AND_OPERATIONS_ZH.md',
  'docs/03_TASK_PROTOCOL_ZH.md','docs/04_SECURITY_AND_GOVERNANCE_ZH.md',
  'docs/05_ACCEPTANCE_ZH.md','docs/11_THINKING_POLICY_DESIGN_ZH.md',
  'docs/12_SPLIT_MEMORY_SCHEMA4_MIGRATION_ZH.md','docs/15_SOURCE_PROVENANCE_ZH.md',
]:
...
assert json.load(open(os.path.join(root,'bridge/package.json'),encoding='utf-8'))['version']==expected_version
assert json.load(open(os.path.join(root,'.codex-plugin/plugin.json'),encoding='utf-8'))['version']==expected_version
assert json.load(open(os.path.join(root,'harness/minimal/MANAGED_MARKER.json'),encoding='utf-8'))['version']==expected_version
if release['releaseStatus']=='stable':
    assert expected_version=='0.6.6'
    ...
    assert release['deliverableStatus']!='DELIVERABLE_PASS'
print(json.dumps({
  "result":"PASS","version":expected_version,"freshInstall":True,"installedAcceptance":True,
  ...
  "releaseMetadataValidated":True,"releaseStatus":release['releaseStatus'],
  "requiredEvidenceBindingsValidated":release['releaseStatus']=='stable',
  "currentGovernanceDocsPresent":True
},indent=2))
PY
