...
import { allowedToolNamesFromRequest, parseDsmlToolCalls, parseMarkdownShellToolCall, parseTextualToolCallEnvelope, transformProviderToolCalls, } from "../tool-call-recovery.js";
const request = {
...
test("recovers canonical full-width DSML content into native streamed tool calls", () => {
    ...
    const original = sse({ id: "chatcmpl-1", model: "deepseek-v4-flash", choices: [{ index: 0, delta: { content: dsml }, finish_reason: "stop" }] }, { id: "chatcmpl-1", model: "deepseek-v4-flash", choices: [], usage: { prompt_tokens: 100, completion_tokens: 20 } });
...
test("normalizes null continuation fields in structured streamed tool calls", () => {
    const original = sse({ id: "chatcmpl-2", choices: [{ index: 0, delta: { tool_calls: [{ index: 0, id: "call-real", type: "function", function: { name: "bash", arguments: "{\"command\":" } }] }, finish_reason: null }] }, { id: "chatcmpl-2", choices: [{ index: 0, delta: { tool_calls: [{ index: 0, id: null, function: { name: null, arguments: "\"echo ok\"}" } }] }, finish_reason: "tool_calls" }] });
    const transformed = transformProviderToolCalls("text/event-stream", original, request);
    assert.equal(transformed.failure, undefined);
...
test("recovers DSML from a non-stream JSON completion", () => {
    ...
    assert.equal(transformed.failure, undefined);
    const root = JSON.parse(transformed.body.toString("utf8"));
...
test("recovers exact XML and bracket textual tool-call envelopes", () => {
    ...
    const xml = parseTextualToolCallEnvelope(`<tool_call name="bash">{"command":"touch probe.json"}</tool_call>`, allowed, boundedRequest);
    assert.equal(xml.failure, undefined);
    assert.equal(xml.calls[0]?.name, "bash");
    const bracket = parseTextualToolCallEnvelope(`[Calling tool: bash with arguments: {"command":"touch probe.json"}]`, allowed, boundedRequest);
...
test("does not treat ordinary target JSON as executable tool-call JSON", () => {
    const parsed = parseTextualToolCallEnvelope(`{"status":"PASS","executor":"harness"}`, allowedToolNamesFromRequest(boundedRequest), boundedRequest);
    assert.equal(parsed.markerFound, false);
    assert.equal(parsed.calls.length, 0);
});
test("fails closed on prose-wrapped textual tool-call markup", () => {
    const parsed = parseTextualToolCallEnvelope(`I will run this now:\n<tool_call name="bash">{"command":"touch probe.json"}</tool_call>`, allowedToolNamesFromRequest(boundedRequest), boundedRequest);
    assert.equal(parsed.markerFound, true);
    assert.match(parsed.failure ?? "", /exact supported envelope/u);
});
//# sourceMappingURL=tool-call-recovery.test.js.map