...
import { appendThinkingEvidence, captureReasoningRequirement, createExecutionAttempt, ensureAttemptThinkingPolicy, preflightThinkingRequest, thinkingPolicyForModel, } from "../thinking-policy.js";
function task(model) {
    return {
        executor: "harness",
...
function proRequest(messages = [{ role: "user", content: "perform the bounded task" }]) {
    return {
...
test("rejects Pro mode switching and tool_choice before Provider I/O", () => {
    ...
    assert.equal(preflightThinkingRequest(current, proRequest(), "deepseek-v4-pro", "2026-08-22T00:00:01.000Z").ok, true);
    for (const patch of [
        { thinking: { type: "disabled" }, reasoning_effort: undefined },
        { thinking: { type: "enabled" }, reasoning_effort: "low" },
        { thinking: { type: "enabled" }, reasoning_effort: "high", tool_choice: "auto" },
    ]) {
        const request = { ...proRequest(), ...patch };
...
test("captures real Pro reasoning integrity and requires exact full-history replay", () => {
    ...
    const captured = captureReasoningRequirement("text/event-stream", capture, first.evidence.attemptId, first.evidence.requestOrdinal, "2026-08-22T00:00:02.000Z");
    assert.equal(captured.ok, true);
    if (!captured.ok || captured.requirement === undefined)
        return;
...
//# sourceMappingURL=thinking-policy.test.js.map