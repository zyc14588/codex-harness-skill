...
async function makeTask(config, repo, baseCommit, root, id, verificationCommand) {
...
test("authoritative verification excludes ignored false-pass artifacts and removes its worktree", async () => {
    ...
    try {
        ...
        const rejected = await verifyTask(falsePassTask.id, undefined, 30);
        ...
        const accepted = await verifyTask(cleanPassTask.id, undefined, 30);
        ...
        if (falsePassTask)
            await removeWorktree(repo, falsePassTask.worktreePath, true).catch(() => undefined);
        if (cleanPassTask)
            await removeWorktree(repo, cleanPassTask.worktreePath, true).catch(() => undefined);
        if (previousConfig === undefined)
            delete process.env.CODEX_HARNESS_CONFIG;
        else
            process.env.CODEX_HARNESS_CONFIG = previousConfig;
        await rm(root, { recursive: true, force: true });
    }
});
//# sourceMappingURL=verification-isolation.test.js.map