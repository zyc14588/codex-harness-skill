...
import { ATTEMPT_PROTOCOL_FAILURE_HTTP_STATUS, attemptInfrastructureAbortReason, } from "../infrastructure-failure.js";
...
async function waitForMonitor(port, child, operatorToken) {
...
async function consume(_request) {
    for await (const _chunk of _request) { /* drain request */ }
}
async function socketJson(socketPath, requestPath, token, body) {
...
function invalidThinkingToolCall(response) {
...
test("Provider protocol is fail-fast and missing reasoning replay reaches no Provider, Token, or split memory", async () => {
    ...
    const proxyPath = `/provider/${taskId}/${attempt.id}/chat/completions`;
    ...
    try {
        ...
        const replayTaskId = "reasoning-replay-pre-provider-fail-fast";
        const replayAttempt = createExecutionAttempt("harness", "deepseek-v4-pro", 1, now);
        assert.ok(replayAttempt.id);
        const replayTaskDir = taskDirectory(config, replayTaskId);
        const replayTask = {
            ...structuredClone(task),
            id: replayTaskId,
            planId: `${replayTaskId}-plan`,
            leafId: `${replayTaskId}-leaf`,
            budgetGroupId: replayTaskId,
            taskFamily: "test/reasoning-replay-pre-provider-fail-fast",
            splitDecision: {
                ...structuredClone(task.splitDecision),
                memoryKey: "reasoning-replay-pre-provider-fail-fast-memory",
                taskFamily: "test/reasoning-replay-pre-provider-fail-fast",
            },
            branchName: `test/${replayTaskId}`,
            promptPath: path.join(replayTaskDir, "prompt.md"),
            stdoutPath: path.join(replayTaskDir, "stdout.log"),
            stderrPath: path.join(replayTaskDir, "stderr.log"),
            usagePath: path.join(replayTaskDir, "usage.ndjson"),
            proxyToken: "b".repeat(48),
            executionAttempts: [replayAttempt],
            providerRequestOrdinal: 0,
            thinkingRequestEvidence: [],
            reasoningReplayRequirements: [{
                    schemaVersion: 1,
                    attemptId: replayAttempt.id,
                    responseRequestOrdinal: 1,
                    reasoningSha256: "f".repeat(64),
                    reasoningUtf8Bytes: 64,
                    toolCallIds: ["call-replay-required"],
                    recordedAt: now,
                    replayCount: 0,
                }],
        };
        await createTask(config, replayTask);
        const callsBeforeReplayOmission = providerCalls;
        const replayRejected = await socketJson(socketPath, `/provider/${replayTaskId}/${replayAttempt.id}/chat/completions`, replayTask.proxyToken, requestBody);
        assert.equal(replayRejected.status, 502);
        assert.equal(replayRejected.body.error?.type, "thinking_replay_state");
        assert.match(replayRejected.body.error?.message ?? "", /assistant tool-call history.*is missing/u);
        assert.equal(providerCalls, callsBeforeReplayOmission, "missing replay reached the Provider");
        const replayUsage = await usageForBudgetGroup(config, replayTaskId);
        assert.equal(replayUsage.apiCalls, 0);
        assert.equal(replayUsage.inputTokens + replayUsage.estimatedInputTokens, 0);
        assert.equal(replayUsage.outputTokens + replayUsage.estimatedOutputTokens, 0);
        const replayStored = await loadTask(config, replayTaskId);
        assert.equal(replayStored.providerRequestOrdinal, 0);
        assert.equal(replayStored.thinkingRequestEvidence?.length, 0);
        assert.equal(replayStored.infrastructureFailureKind, "thinking_replay_state");
        assert.equal(replayStored.splitOutcomeRecordedAt, undefined);
        assert.equal(replayStored.splitOutcomeRevision, undefined);
        assert.deepEqual(await listSplitMemoryProfiles(config, root), []);
    }
    finally {
...
//# sourceMappingURL=provider-protocol-fail-fast.test.js.map