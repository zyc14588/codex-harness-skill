...
async function ensureAllowedRepo(config, candidate) {
...
function validateRuntime(config, seconds) {
...
async function harnessRevision(config) {
    ...
    const dirty = await runProcess("git", ["-C", config.harnessRoot, "status", "--porcelain=v1", "--untracked-files=no"], { timeoutMs: 10_000 });
    if (dirty.code !== 0) {
        return {
            pinned: config.pinnedHarnessCommit,
...
async function harnessBuildIntegrity(config, launcherSource) {
...
export async function assertHarnessProvenance(config) {
...
async function spawnWorker(config, task) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            if (!workerPid)
                throw new Error("worker spawned without a PID");
            ...
            while (Date.now() < deadline) {
                ...
                if (!await processIdentityMatches(workerIdentity))
                    break;
            ...
            if (current.status !== "queued")
                throw new Error(`${lastFailure}; task status is ${current.status}`);
            await updateTask(config, task.id, (latest) => {
                if (latest.status !== "queued")
                    throw new Error(`cannot retry worker bootstrap over status ${latest.status}`);
                if (latest.workerPid === workerPid)
                    delete latest.workerPid;
                if (latest.workerIdentity?.startTimeTicks === workerIdentity.startTimeTicks)
                    delete latest.workerIdentity;
            ...
            await updateTask(config, task.id, (latest) => {
                if (latest.status !== "queued")
                    throw new Error(`cannot retry worker bootstrap over status ${latest.status}`);
                if (latest.workerPid === child.pid)
                    delete latest.workerPid;
                if (latest.workerIdentity?.pid === child.pid)
                    delete latest.workerIdentity;
                delete latest.workerDeadObservedAt;
            });
...
async function snapshotFingerprint(worktreePath, baseCommit) {
    const patch = await binaryPatch(worktreePath, baseCommit);
    const paths = await changedPaths(worktreePath, baseCommit);
    return createHash("sha256")
        .update(baseCommit).update("\0")
        .update(paths.join("\0")).update("\0")
        .update(patch)
        .digest("hex");
}
async function changeFingerprint(task) {
    await assertTaskWorktreeIdentity(task);
    return await snapshotFingerprint(task.worktreePath, task.baseCommit);
}
function terminal(task) {
    return task.status !== "queued" && task.status !== "running";
}
function publicTaskSummary(task) {
    return {
        id: task.id,
        planId: task.planId,
        leafId: task.leafId,
...
function validatedHarnessUpstreamBaseUrl(config) {
    ...
    try {
        ...
        throw new Error("provider.baseUrl must be a valid absolute URL");
    }
    ...
    if (value.origin === monitor.origin)
        throw new Error("DEEPSEEK_BASE_URL must not point at the bridge monitor; recursive proxying is forbidden");
    return raw;
}
async function terminateRecordedProcess(identity) {
    if (!identity || identity.pid === process.pid)
        return;
    if (!await signalVerifiedProcessGroup(identity, "SIGTERM"))
        return;
    const escalation = setTimeout(() => { void signalVerifiedProcessGroup(identity, "SIGKILL"); }, 5_000);
    escalation.unref();
}
async function assertWorktreeIdle(config, worktreePath) {
    const active = (await listTasks(config)).find((candidate) => candidate.worktreePath === worktreePath && (candidate.status === "queued" || candidate.status === "running"));
    if (active)
        throw new Error(`worktree has active task ${active.id}`);
}
function expectedTaskHead(task) {
    return task.bridgeCommit ?? task.startingHeadCommit ?? task.baseCommit;
}
async function assertTaskHeadExpected(task, operation) {
...
function assertAdoptableStatus(task, operation) {
...
function checkedHarnessPrompt(task, repairFeedback) {
...
function validateBudgetAgainstMaximum(defaults, maximum, requested) {
    const budget = {
...
function validateProComplexBudget(defaults, requested) {
    return {
        ...normalizeTaskBudget(requested, defaults, "leaf.budget"),
        gatePolicy: "input_output_tokens",
...
const PROGRESSIVE_CAPABILITIES = new Set(["repository_read", "verification", "git_inspect"]);
const COMPLEXITY_ORDER = ["trivial", "small", "medium", "large"];
function complexityRank(value) {
    return COMPLEXITY_ORDER.indexOf(value);
}
function defaultTaskFamily(mode, leases, executor, model) {
...
function normalizeToolCapabilities(value, minimal) {
    const selected = value ?? (minimal ? ["repository_read", "verification", "git_inspect"] : []);
    const unique = [...new Set(selected)];
    for (const capability of unique) {
        if (!PROGRESSIVE_CAPABILITIES.has(capability))
            throw new Error(`unsupported progressive tool capability: ${capability}`);
    }
    return unique;
}
async function normalizedLeaf(config, repoRoot, defaults, maximum, proComplexDefaults, llama, input) {
    ...
    if (!harnessWritePaths.length)
        throw new Error(`leaf ${id} requires at least one worker write lease`);
    ...
    if (harnessWritePaths.includes("**"))
        throw new Error(`leaf ${id} cannot lease the entire repository`);
    assertDisjointLeases(harnessWritePaths, codexWritePaths);
    let executor;
    let routingReason;
    if (requestedExecutor === "llama_cpp") {
        if (!llama.enabled)
            throw new Error(`leaf ${id}: llama.cpp was explicitly selected but is disabled in operator controls`);
        if (complexity !== "trivial" && complexity !== "small")
            throw new Error(`leaf ${id}: llama.cpp is restricted to trivial or small leaves`);
    ...
    if (dependsOn.includes(id))
        throw new Error(`leaf ${id} cannot depend on itself`);
    const taskFamily = boundedText(input.taskFamily?.trim() || defaultTaskFamily(mode, harnessWritePaths, executor, selectedModel), `leaf ${id} taskFamily`, 500);
    const splitRationale = boundedText(input.splitRationale?.trim() || "Codex selected this bounded leaf from the frozen objective, write lease, model tier, and verification contract.", `leaf ${id} splitRationale`, 8_000);
    ...
    const adaptiveBudget = { ...(input.budget ?? {}) };
    ...
    if (memoryOverrideReason)
        advice.decision.overrideReason = memoryOverrideReason;
    ...
    if (!acceptanceCriteria.length)
        throw new Error(`leaf ${id} requires acceptance criteria`);
    ...
    if (!verificationCommands.length)
        throw new Error(`leaf ${id} requires authoritative verification commands`);
...
function validateLeafDependencies(leaves) {
    ...
    for (const leaf of leaves) {
        for (const dependency of leaf.dependsOn) {
            ...
            if (!target)
                throw new Error(`leaf ${leaf.id} depends on unknown leaf ${dependency}`);
    ...
    const visit = (id) => {
        if (visited.has(id))
            return;
        if (visiting.has(id))
            throw new Error(`controller leaf dependency cycle detected at ${id}`);
        visiting.add(id);
...
export async function createControllerPlan(input) {
    ...
    return await withMutationLock(config, async () => {
        ...
        if (config.requireCleanRepoAtStart) {
            ...
            if (dirty.length)
                throw new Error(`repository must be clean while freezing the controller base commit: ${dirty.slice(0, 20).join(", ")}`);
        }
        ...
        const leaves = await Promise.all(input.leaves.map(async (leaf) => await normalizedLeaf(config, repoRoot, policy.defaultHarnessBudget, policy.maximumHarnessBudget, policy.defaultProComplexBudget, llama, leaf)));
        if (new Set(leaves.map((leaf) => leaf.id)).size !== leaves.length)
            throw new Error("controller leaf IDs must be unique");
...
export async function controllerPlanStatus(planId) {
    const config = await loadConfig();
...
export async function listControllerPlans(limit) {
    const config = await loadConfig();
...
export async function controllerSplitAdvice(repoRootInput, candidates) {
    const config = await loadConfig();
...
export async function controllerSplitMemory(repoRootInput) {
    const config = await loadConfig();
...
function effectiveConfigEntry(dump, id) {
...
export function inspectMinimalProfileComposition(stdout, stderr) {
    ...
    const errors = [];
    if (!stockRunnerDisabled)
        errors.push("effective headless-runner is not disabled");
    if (!bridgeRunnerMounted)
        errors.push("effective codex-bridge-headless-runner is not mounted");
    if (!sessionTitleDisabled)
        errors.push("effective session-title-llm is not disabled");
    if (!minimalPresetSelected)
        errors.push("effective agent-presets.default is not codex-bridge-minimal");
    if (!patchWarningFree)
        errors.push("dump-config reported a patch mismatch/skip warning");
    return {
        ok: errors.length === 0,
        stockRunnerDisabled,
        bridgeRunnerMounted,
...
export async function doctor(probeHarness) {
    const config = await loadConfig();
    ...
    const brokerHostPath = fileURLToPath(new URL("./brokered-tool-host.js", import.meta.url));
    const brokerPluginPath = path.join(minimalProfilePath, "bridge-brokered-tools.mjs");
    const minimalRequestStatePath = fileURLToPath(new URL("./minimal-request-state.js", import.meta.url));
    const minimalPresetBroker = await inspectMinimalPresetBrokerComposition(minimalPresetPath, undefined, config.harnessMinimalProfile);
    const minimalIntegration = {
        ...
        brokeredToolPlugin: brokerPluginPath,
        brokeredToolPluginExists: await pathExists(brokerPluginPath),
        brokerHostModule: brokerHostPath,
        brokerHostModuleExists: await pathExists(brokerHostPath),
        requestStateModule: minimalRequestStatePath,
        requestStateModuleExists: await pathExists(minimalRequestStatePath),
        brokerComposition: minimalPresetBroker,
    };
    const checks = {
    ...
    for (const root of config.allowedRepoRoots) {
        try {
            rootChecks.push({ root, ok: true, canonical: await realpath(root) });
        }
        catch (error) {
            rootChecks.push({ root, ok: false, error: error instanceof Error ? error.message : String(error) });
        }
    }
    ...
    try {
        ...
        if (config.enforceHarnessBuildHash && !buildIntegrity.matches)
            launcherOk = false;
        if (probeHarness) {
        ...
        checks.harnessLauncher = { ok: false, error: error instanceof Error ? error.message : String(error) };
    }
    ...
    if (config.monitor.enabled) {
        try {
            ...
            checks.monitorStatus = { ok: false, error: error instanceof Error ? error.message : String(error) };
        }
    }
...
export async function startTask(input) {
    ...
    return await withMutationLock(config, async () => {
        ...
        if (plan.status === "accepted" || plan.status === "rejected")
            throw new Error(`cannot launch a leaf from finalized plan ${plan.status}`);
        const leafId = safeTaskId(input.leafId);
        const leaf = plan.leaves.find((candidate) => candidate.id === leafId);
        if (!leaf)
            throw new Error(`leaf not found in plan: ${leafId}`);
        if (leaf.status !== "planned")
            throw new Error(`leaf ${leafId} cannot launch from status ${leaf.status}`);
        ...
        const runtimeLlama = await effectiveLlamaConfig(config);
        if (leaf.executor === "harness" || (leaf.executor === "llama_cpp" && runtimeLlama.fallbackEnabled))
        ...
        const repoRoot = await ensureAllowedRepo(config, await gitTopLevel(requestedRoot));
        if (await resolveCommit(repoRoot, plan.baseCommit) !== plan.baseCommit)
            throw new Error("controller base commit no longer resolves");
        if (config.requireCleanRepoAtStart) {
            const dirtyPaths = await workingTreePaths(repoRoot);
            if (dirtyPaths.length)
                throw new Error(`repository must remain clean until the worker leaf is launched: ${dirtyPaths.slice(0, 20).join(", ")}`);
        }
        ...
        for (const existing of activeInRepo) {
            try {
                assertDisjointLeases(existing.harnessWritePaths, leaf.harnessWritePaths);
            }
            catch {
                throw new Error(`leaf ${leafId} write leases overlap active task ${existing.id}`);
            }
        }
        const id = safeTaskId(input.taskId ?? `${plan.id}-${leaf.id}`);
        if (await pathExists(taskFile(config, id)))
        ...
        if (symlinkIntersections.length)
            throw new Error(`worker leases intersect tracked symlinks: ${symlinkIntersections.join(", ")}`);
        ...
        if (gitlinkIntersections.length)
            throw new Error(`worker leases intersect tracked gitlinks/submodules: ${gitlinkIntersections.join(", ")}`);
        ...
        if (leaf.executor === "harness" || runtimeLlama.fallbackEnabled)
        ...
        try {
            ...
            const adapterToken = proxyToken ? randomBytes(32).toString("hex") : undefined;
            const toolToken = proxyToken ? randomBytes(32).toString("hex") : undefined;
            task = {
                schemaVersion: 6,
                id,
                planId: plan.id,
                ...
                changedPaths: [],
                outOfScopePaths: [],
                ...(leaf.harnessMode === "minimal" ? { minimalRequestPhase: "booting" } : {}),
            ...
            await updatePlan(config, plan.id, (current) => {
                ...
                if (!selected || selected.status !== "planned")
                    throw new Error("controller leaf changed during launch");
            ...
            if (task && await pathExists(taskFile(config, task.id))) {
                task = await updateTask(config, task.id, (current) => {
                    if (current.status !== "cancelled") {
                        ...
                        current.error = `launch failed: ${error instanceof Error ? error.message : String(error)}`;
                    }
                });
                if (worktreeCreated) {
                    try {
...
export async function taskStatus(taskId) {
    ...
    const livenessDecision = decideWorkerLiveness(taskActive, workerAlive, task.workerDeadObservedAt, Date.now(), WORKER_ORPHAN_GRACE_MS);
    if (livenessDecision === "observe-dead" || livenessDecision === "clear-dead-observation" || livenessDecision === "orphan") {
        task = await updateTask(config, id, (current) => {
            ...
            const currentDecision = decideWorkerLiveness(currentActive, currentAlive, current.workerDeadObservedAt, Date.now(), WORKER_ORPHAN_GRACE_MS);
...
export async function collectTask(taskId, includePatch, maxPatchChars) {
    const config = await loadConfig();
    const id = safeTaskId(taskId);
    ...
    return await withWorktreeLock(config, initial.worktreePath, async () => {
        ...
        if (!terminal(task))
            throw new Error("cannot collect a stable patch while task is active; use harness_status");
...
export async function readChangedFile(taskId, filePath) {
    ...
    return await withWorktreeLock(config, initial.worktreePath, async () => {
        const task = await loadTask(config, id);
        if (!terminal(task))
            throw new Error("cannot read a stable changed file while task is active");
        await assertWorktreeIdle(config, task.worktreePath);
        await assertTaskWorktreeIdentity(task);
...
export async function reviewTask(taskId, decision, reviewedPaths, notes) {
    ...
    return await withWorktreeLock(config, initial.worktreePath, async () => {
        ...
        await updatePlan(config, task.planId, (plan) => {
            ...
            if (!leaf)
                throw new Error(`plan leaf missing during review: ${task.leafId}`);
...
export async function repairTask(parentTaskId, feedback, runtimeSeconds) {
    ...
    return await withMutationLock(config, async () => {
        ...
        const runtimeLlama = await effectiveLlamaConfig(config);
        if (initial.executor === "harness" || (initial.executor === "llama_cpp" && runtimeLlama.fallbackEnabled))
            await assertHarnessProvenance(config);
        if (initial.executor === "harness" || runtimeLlama.fallbackEnabled) {
            await readProviderApiKey(config);
            if (!config.monitor.enabled)
                throw new Error("Harness repair requires the credential-brokering monitor proxy");
            await ensureMonitorRunning(config, defaultConfigPath());
        }
        return await withWorktreeLock(config, initial.worktreePath, async () => {
            const parent = await loadTask(config, parentId);
            if (!terminal(parent))
                throw new Error("cannot repair while parent task is active");
            if (parent.reviewDecision !== "revise")
                throw new Error("repair requires a Codex review decision of revise");
            if (parent.worktreeRemoved)
                throw new Error("cannot repair a task whose worktree was removed");
            ...
            if (exhausted)
                throw new Error(`cannot launch repair because the cumulative leaf budget is exhausted: ${exhausted}`);
            ...
            if (await pathExists(taskFile(config, id)))
                throw new Error(`repair task already exists: ${id}`);
            const dir = taskDirectory(config, id);
            await ensureDir(dir);
            const task = {
                ...parent,
                ...
                ...(config.monitor.enabled && (parent.executor === "harness" || runtimeLlama.fallbackEnabled) ? {
                    ...
                    toolToken: randomBytes(32).toString("hex"),
                    dashboardUrl: `${monitorBaseUrl(config)}/#task=${encodeURIComponent(id)}`,
            ...
            if (task.harnessMode === "minimal")
                task.minimalRequestPhase = "booting";
            else
                delete task.minimalRequestPhase;
            ...
            await updatePlan(config, task.planId, (plan) => {
                const leaf = plan.leaves.find((candidate) => candidate.id === task.leafId);
                if (!leaf)
                    throw new Error(`plan leaf missing during repair: ${task.leafId}`);
                leaf.status = "running";
                leaf.activeTaskId = task.id;
            ...
            try {
                ...
                await updateTask(config, task.id, (current) => {
                    if (current.status !== "cancelled") {
                        current.status = "failed";
                        current.completedAt = nowIso();
                        current.error = `repair launch failed: ${error instanceof Error ? error.message : String(error)}`;
                    }
                });
                throw error;
            }
...
export async function verifyTask(taskId, commands, timeoutSeconds = 1800) {
    const config = await loadConfig();
    const id = safeTaskId(taskId);
    const initial = await loadTask(config, id);
    return await withWorktreeLock(config, initial.worktreePath, async () => {
        ...
        if (!Number.isInteger(timeoutSeconds) || timeoutSeconds < 1 || timeoutSeconds > 7200)
            throw new Error("timeoutSeconds must be an integer from 1 to 7200");
        ...
        if (!selected.length)
            throw new Error("controller plan contains no verification commands");
        ...
        if (beforeViolations.length)
            throw new Error(`cannot verify out-of-scope changes: ${beforeViolations.join(", ")}`);
        const beforeUnsafeSymlinks = await unsafeChangedSymlinkPaths(task.worktreePath, beforePaths);
        if (beforeUnsafeSymlinks.length)
            throw new Error(`cannot verify changed symlink paths: ${beforeUnsafeSymlinks.join(", ")}`);
        const beforeUnsafeGitlinks = await unsafeChangedGitlinkPaths(task.worktreePath, beforePaths);
        if (beforeUnsafeGitlinks.length)
            throw new Error(`cannot verify changed gitlink/submodule metadata: ${beforeUnsafeGitlinks.join(", ")}`);
        const beforeStaged = await stagedPaths(task.worktreePath);
        if (beforeStaged.length)
            throw new Error(`cannot verify worker-staged index changes: ${beforeStaged.join(", ")}`);
        ...
        const reviewedPatch = await binaryPatch(task.worktreePath, task.baseCommit);
        const reviewedPatchSha256 = createHash("sha256").update(reviewedPatch).digest("hex");
        const sourceIgnoredResidue = await ignoredUntrackedPaths(task.worktreePath);
        ...
        let verificationWorktreeRemoved = false;
        let cleanStart = false;
        let appliedPaths = [];
        let finalPaths = [];
        let finalViolations = [];
        let finalUnsafeSymlinks = [];
        let finalUnsafeGitlinks = [];
        let finalStaged = [];
        let finalFingerprint = "";
        let verificationHeadStable = false;
        let verificationIgnoredAfterCommands = 0;
        try {
            await createDetachedVerificationWorktree(task.repoRoot, verificationDirectory, task.baseCommit);
            verificationCreated = true;
            await cleanWorktreeIncludingIgnored(verificationDirectory);
            cleanStart = (await ignoredUntrackedPaths(verificationDirectory)).length === 0
                && (await workingTreePaths(verificationDirectory)).length === 0
                && await resolveCommit(verificationDirectory, "HEAD") === task.baseCommit;
            if (!cleanStart)
                throw new Error("fresh verification worktree is not a clean checkout of baseCommit");
            if (reviewedPatch.length > 0)
                await applyReviewedPatch(verificationDirectory, reviewedPatchPath);
            appliedPaths = await changedPaths(verificationDirectory, task.baseCommit);
            if (JSON.stringify(appliedPaths) !== JSON.stringify(beforePaths)) {
                throw new Error(`reviewed patch reconstructed unexpected paths: ${appliedPaths.join(", ")}`);
            }
            const appliedFingerprint = await snapshotFingerprint(verificationDirectory, task.baseCommit);
            ...
            verificationHeadStable = await resolveCommit(verificationDirectory, "HEAD") === task.baseCommit;
            finalPaths = await changedPaths(verificationDirectory, task.baseCommit);
            finalViolations = findOutOfScope(finalPaths, task.harnessWritePaths);
            finalUnsafeSymlinks = await unsafeChangedSymlinkPaths(verificationDirectory, finalPaths);
            finalUnsafeGitlinks = await unsafeChangedGitlinkPaths(verificationDirectory, finalPaths);
            finalStaged = await stagedPaths(verificationDirectory);
            verificationIgnoredAfterCommands = (await ignoredUntrackedPaths(verificationDirectory)).length;
            finalFingerprint = await snapshotFingerprint(verificationDirectory, task.baseCommit);
        }
        finally {
            if (verificationCreated) {
                await removeWorktree(task.repoRoot, verificationDirectory, true);
                verificationWorktreeRemoved = !await pathExists(verificationDirectory);
            }
        }
        await assertTaskWorktreeIdentity(task);
        await assertTaskHeadExpected(task, "verification");
        ...
        task.verificationWorktreeRemoved = verificationWorktreeRemoved;
        ...
        if (!fingerprintStable)
            task.error = "fresh authoritative verification or the agent tree differs from the reviewed snapshot; re-review before adoption";
        await writeFile(verificationEvidencePath, `${JSON.stringify({
            ...
            reviewedFingerprint,
            currentFingerprint,
            verifiedFingerprint: finalFingerprint,
            verificationResultFingerprint,
            sourceIgnoredResidueExcluded: sourceIgnoredResidue.length,
        ...
        await updatePlan(config, task.planId, (plan) => {
            const leaf = plan.leaves.find((candidate) => candidate.id === task.leafId);
            if (!leaf)
                throw new Error(`plan leaf missing during verification: ${task.leafId}`);
            leaf.completedTaskId = task.id;
            if (passed) {
        ...
        try {
            await recordTaskSplitOutcome(config, task, "verification");
        }
        catch { /* verification result remains authoritative */ }
        return {
...
export async function commitTask(taskId, message) {
    ...
    return await withWorktreeLock(config, initial.worktreePath, async () => {
        ...
        if (task.reviewDecision !== "approved" || !task.reviewedFingerprint)
            throw new Error("commit requires approved Codex review");
        if (!task.verificationPassed || !task.verifiedFingerprint)
            throw new Error("commit requires passing authoritative verification");
        if (task.reviewedFingerprint !== task.verifiedFingerprint)
            throw new Error("reviewed and verified fingerprints differ; adoption is forbidden");
        await assertWorktreeIdle(config, task.worktreePath);
        await assertTaskWorktreeIdentity(task);
        const currentHead = await resolveCommit(task.worktreePath, "HEAD");
        if (task.bridgeCommit) {
            const bridgeCommit = task.bridgeCommit;
            if (currentHead !== bridgeCommit)
                throw new Error(`recorded bridge commit ${bridgeCommit} does not match current HEAD ${currentHead}`);
            const dirtyPaths = await workingTreePaths(task.worktreePath);
            if (dirtyPaths.length)
                throw new Error(`recorded bridge commit has uncommitted worktree changes: ${dirtyPaths.join(", ")}`);
            const currentFingerprint = await changeFingerprint(task);
            if (currentFingerprint !== task.verifiedFingerprint)
                throw new Error("recorded bridge commit differs from the reviewed and verified snapshot");
            await updatePlan(config, task.planId, (plan) => {
                const leaf = plan.leaves.find((candidate) => candidate.id === task.leafId);
                if (leaf) {
                    leaf.status = "accepted";
                    leaf.bridgeCommit = bridgeCommit;
                    leaf.completedTaskId = task.id;
                }
            });
        ...
        const staged = await stagedPaths(task.worktreePath);
        if (staged.length)
            throw new Error(`refusing to commit worker-staged index changes: ${staged.join(", ")}`);
        const paths = await changedPaths(task.worktreePath, task.baseCommit);
        const violations = findOutOfScope(paths, task.harnessWritePaths);
        if (violations.length)
            throw new Error(`refusing to commit out-of-scope paths: ${violations.join(", ")}`);
        const unsafeSymlinks = await unsafeChangedSymlinkPaths(task.worktreePath, paths);
        if (unsafeSymlinks.length)
            throw new Error(`refusing to commit changed symlink paths: ${unsafeSymlinks.join(", ")}`);
        const unsafeGitlinks = await unsafeChangedGitlinkPaths(task.worktreePath, paths);
        if (unsafeGitlinks.length)
            throw new Error(`refusing to commit changed gitlink/submodule metadata: ${unsafeGitlinks.join(", ")}`);
        const currentFingerprint = await changeFingerprint(task);
        ...
        await updatePlan(config, task.planId, (plan) => {
            const leaf = plan.leaves.find((candidate) => candidate.id === task.leafId);
            if (!leaf)
                throw new Error(`plan leaf missing during commit: ${task.leafId}`);
            leaf.status = "accepted";
...
export async function cancelTask(taskId) {
    ...
    if (task.status !== "cancelled")
        return { taskId: task.id, status: task.status, changed: false };
    await Promise.all([
...
export async function cleanupTask(taskId, force, deleteTaskBranch) {
    const config = await loadConfig();
    return await withMutationLock(config, async () => {
        const id = safeTaskId(taskId);
        const initial = await loadTask(config, id);
        return await withWorktreeLock(config, initial.worktreePath, async () => {
            const task = await loadTask(config, id);
            if (!terminal(task))
                throw new Error("cannot clean up an active task");
            if (task.status === "cancelled" && (await processIdentityMatches(task.workerIdentity) || await processIdentityMatches(task.harnessIdentity))) {
            ...
            const persistFlags = async (cleanedAt) => {
                for (const candidate of related) {
                    if (cleanedAt)
                        candidate.cleanedAt = cleanedAt;
                    if (task.worktreeRemoved)
                        candidate.worktreeRemoved = true;
                    if (task.branchDeleted)
            ...
            if (!task.worktreeRemoved) {
                ...
                if (!task.worktreeRemoved)
                    throw new Error(`worktree still exists after cleanup attempt: ${task.worktreePath}`);
...
export async function listRecentTasks(limit) {
    const config = await loadConfig();
...
export async function finalizeControllerPlan(planId, integrationEvidence) {
    const config = await loadConfig();
    ...
    return await withMutationLock(config, async () => {
        ...
        if (plan.status === "rejected")
            throw new Error("a rejected controller plan cannot be finalized as accepted");
        ...
        if (incomplete.length)
            throw new Error(`all leaves must be reviewed, verified, and accepted before finalization: ${incomplete.map((leaf) => `${leaf.id}:${leaf.status}`).join(", ")}`);
...
export async function monitorSnapshot(limit) {
    const config = await loadConfig();
...
export async function monitorStop() {
    const config = await loadConfig();
