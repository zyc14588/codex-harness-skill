...
import { cleanupTask, createControllerPlan, inspectMinimalProfileComposition, monitorStop, startTask, taskStatus, } from "./service.js";
...
async function waitTerminal(taskId, timeoutMs = 120_000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const current = await taskStatus(taskId);
        if (!new Set(["queued", "running"]).has(String(current.status)))
            return current;
        await sleep(100);
    }
...
const profileModulesSource = path.resolve(process.env.CODEX_HARNESS_FIXTURE_PROFILE_MODULES
    ?? "/home/zyc14588/.dsh/profiles/node_modules");
...
let providerFailure;
let providerPort = 0;
function assistantReasoning(body) {
...
function toolResultSummaries(body) {
    if (!Array.isArray(body.messages))
        return [];
    return body.messages.flatMap((raw) => {
        if (!raw || typeof raw !== "object" || Array.isArray(raw))
...
function sendToolCall(response, model, responseId, callId, command, reasoning) {
...
function sendText(response, model, responseId, content, reasoning) {
    response.writeHead(200, { "content-type": "text/event-stream", "cache-control": "no-cache" });
    if (reasoning !== undefined) {
        response.write(`data: ${JSON.stringify({
            id: responseId,
...
const provider = http.createServer(async (request, response) => {
    try {
        ...
        if (model === "deepseek-v4-flash") {
            ...
            if (ordinal <= 2)
                assert.equal(body.tool_choice, "required", `Flash request ${ordinal} was not forced before a diff`);
            else
                assert.equal(Object.prototype.hasOwnProperty.call(body, "tool_choice"), false, `Flash request ${ordinal} retained tool_choice after a diff`);
            if (ordinal === 1) {
                const durable = JSON.parse(await readFile(path.join(stateRoot, "tasks", flashTaskId, "task.json"), "utf8"));
            ...
            if (ordinal === 2)
                return sendToolCall(response, model, "dynamic-flash-2", "call-flash-write", `printf 'dynamic-flash-profile\\n' > ${flashTargetPath}`);
            if (ordinal === 3)
                return sendToolCall(response, model, "dynamic-flash-3", "call-flash-verify", `test "$(cat ${flashTargetPath})" = dynamic-flash-profile && git diff --check`);
            if (ordinal === 4)
                return sendText(response, model, "dynamic-flash-4", "Implemented and verified the exact leased Flash file.");
        ...
        if (ordinal === 1)
            return sendToolCall(response, model, "dynamic-pro-1", "call-pro-write", `printf 'dynamic-pro-profile\\n' > ${proTargetPath}`, proReasoning[0]);
        if (ordinal === 2)
            return sendToolCall(response, model, "dynamic-pro-2", "call-pro-verify", `test "$(cat ${proTargetPath})" = dynamic-pro-profile && git diff --check`, proReasoning[1]);
        if (ordinal === 3)
            return sendText(response, model, "dynamic-pro-3", "Implemented and verified the exact leased Pro file.", proReasoning[2]);
        throw new Error(`unexpected Pro request ${ordinal}`);
    }
    catch (error) {
        providerFailure = error instanceof Error ? error.message : String(error);
        if (!response.headersSent)
            json(response, 500, { error: providerFailure });
        else
            response.destroy(error instanceof Error ? error : new Error(providerFailure));
    }
});
...
try {
    ...
    const flashEvidence = flashTerminal.minimalRequestEvidence;
    ...
    const seededCapture = captureReasoningRequirement("application/json", Buffer.from(JSON.stringify({ choices: [{ index: 0, message: {
                    role: "assistant",
    ...
    const injectedResponse = await socketJson(monitorSocketPath(config), `/provider/${injection.id}/${injectionAttempt.id}/chat/completions`, injection.proxyToken, {
        model: "deepseek-v4-pro",
    ...
    if (process.env.KEEP_CODEX_HARNESS_DYNAMIC_FIXTURE !== "1")
        await rm(temp, { recursive: true, force: true });
}
//# sourceMappingURL=dynamic-profile-fixture.js.map