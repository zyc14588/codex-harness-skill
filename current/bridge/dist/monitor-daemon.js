...
import { authorizeBearer, ensureOperatorToken, monitorSocketDirectory, monitorSocketPath, OperatorAuthGuard, readProviderApiKey, replaceOperatorToken, } from "./security.js";
...
function activeAttempt(task, attemptId) {
...
function assertProviderChatSchema(body) {
    const unknown = Object.keys(body).filter((key) => !PROVIDER_CHAT_KEYS.has(key));
    if (unknown.length > 0)
        throw new HttpRequestError(400, `Provider request contains unsupported fields: ${unknown.join(", ")}`);
...
async function proxyRequest(config, request, response, taskId, attemptId) {
    if (request.method !== "POST")
        return json(response, 405, { error: "Provider relay permits POST only" });
    if (!isJsonRequest(request))
        return json(response, 415, { error: "Provider relay requires application/json" });
    let task;
    try {
    ...
    await withNamedLock(config, `budget:${task.budgetGroupId}`, Math.max(30_000, task.runtimeSeconds * 1_000), async () => {
        ...
        try {
            assertProviderChatSchema(parsed);
        }
        catch (error) {
...
function render(s){latest=s;const commit=s.installationIdentity&&s.installationIdentity.implementationCommit;q("#connection").textContent="已连接 · v"+s.serviceVersion+(commit?" · implementation "+commit:"")+" · "+new Date(s.generatedAt).toLocaleString("zh-CN");authenticationSucceeded();renderTasks(s);renderCosts(s);if(!q("#llamaModel").matches(":focus"))fillLlama(s.llamaCpp)}
...
async function operatorAuthorized(request, response) {
    const decision = await operatorAuthGuard.authorize(request.headers.authorization, operatorToken, request.socket.remoteAddress ?? "unknown-local-client");
    if (decision.ok)
        return true;
...
async function authorizedAdapterTask(request, taskId, attemptId) {
    let task;
    try {
        task = await loadTask(config, taskId);
    }
...
async function internalRequestState(request, response, taskId, attemptId, operation) {
    if (request.method !== "POST")
        return json(response, 405, { error: "request-state operations require POST" });
    if (!isJsonRequest(request))
        return json(response, 415, { error: "request-state operations require application/json" });
    const body = await readControlBody(request);
    const task = await authorizedAdapterTask(request, taskId, attemptId);
    if (body.taskId !== task.id)
        throw new HttpRequestError(400, "request-state body taskId does not match its route");
...
const internalServer = http.createServer(async (request, response) => {
    try {
        ...
        if (url.search || url.hash)
            return json(response, 404, { error: "internal routes do not accept query or fragment components" });
        const provider = /^\/provider\/([A-Za-z0-9._-]{1,160})\/([A-Za-z0-9._-]{1,160})\/chat\/completions$/u.exec(url.pathname);
        if (provider?.[1] && provider[2])
            return await proxyRequest(config, request, response, provider[1], provider[2]);
        ...
        if (state?.[1] && state[2] && state[3])
            return await internalRequestState(request, response, state[1], state[2], state[3]);
        const tool = /^\/tool-exec\/([A-Za-z0-9._-]{1,160})\/([A-Za-z0-9._-]{1,160})$/u.exec(url.pathname);
        if (tool?.[1] && tool[2])
            return await brokeredToolRequest(request, response, tool[1], tool[2]);
        return json(response, 404, { error: "not found" });
    }
    catch (error) {
        const status = error instanceof HttpRequestError ? error.status : 500;
