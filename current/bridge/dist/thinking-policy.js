...
export const ATTEMPT_THINKING_POLICY_VERSION = "attempt-thinking-policy-v1";
const MAX_THINKING_EVIDENCE = 128;
export function thinkingPolicyForModel(model, frozenAt) {
...
export function createExecutionAttempt(executor, model, ordinal, startedAt) {
    const value = {
        id: randomUUID(),
        ordinal,
        executor,
 ...
 */
export function ensureAttemptThinkingPolicy(task, model, frozenAt) {
    ...
    if (expected === undefined)
        return `no immutable thinking policy exists for model ${model}`;
...
export function validateReasoningReplay(messages, requirements) {
    if (requirements.length === 0)
        return { ok: true, replayedRequirementOrdinals: [] };
...
export function preflightThinkingRequest(task, body, model, providerSentAt) {
    const attempt = task.executionAttempts?.at(-1);
    ...
    return { ok: true, attempt: attempt, evidence };
}
function responsePackets(contentType, capture) {
...
function firstChoicePayload(packet) {
...
export function captureReasoningRequirement(contentType, capture, attemptId, responseRequestOrdinal, recordedAt) {
    let reasoning = "";
    const ids = new Set();
    ...
    for (const packet of responsePackets(contentType, capture)) {
        ...
        for (const raw of payload.tool_calls) {
            ...
            if (typeof id === "string" && id.length > 0)
                ...
                sawToolCallWithoutId = true;
        }
    }
    if (ids.size === 0 && !sawToolCallWithoutId)
        return { ok: true };
...
export function appendThinkingEvidence(task, evidence) {
    ...
    if (minimal !== undefined) {
        ...
        if (evidence.reasoningEffort !== undefined)
            minimal.providerReasoningEffort = evidence.reasoningEffort;
        else
            delete minimal.providerReasoningEffort;
    ...
    for (const ordinal of evidence.replayedRequirementOrdinals) {
        const requirement = task.reasoningReplayRequirements?.find((candidate) => (candidate.attemptId === evidence.attemptId && candidate.responseRequestOrdinal === ordinal));
...
//# sourceMappingURL=thinking-policy.js.map