...
function monitorEnvironment(config, configPath) {
    const env = {
    ...
    for (const name of ["LC_ALL", "NODE_EXTRA_CA_CERTS", "SSL_CERT_FILE", "LLAMA_CPP_API_KEY"]) {
    ...
    return await withNamedLock(config, "monitor-start-stop", 30_000, async () => {
        ...
        try {
            ...
            if (identity.processGroupId !== identity.pid)
                throw new Error("monitor daemon did not become its process-group leader");
            const record = {
                schemaVersion: 2,
                pid: child.pid,
                identity,
...
export async function stopMonitor(config) {
    return await withNamedLock(config, "monitor-start-stop", 30_000, async () => {
        ...
        while (Date.now() < gracefulDeadline && await processIdentityMatches(identity))
            await sleep(50);
        if (await processIdentityMatches(identity)) {
            await signalVerifiedProcessGroup(identity, "SIGKILL");
            const killDeadline = Date.now() + 1_000;
            while (Date.now() < killDeadline && await processIdentityMatches(identity))
                await sleep(25);
        }
        await rm(monitorPidPath(config), { force: true });
