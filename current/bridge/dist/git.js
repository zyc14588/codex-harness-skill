...
export async function ignoredUntrackedPaths(worktreePath) {
...
export async function createDetachedVerificationWorktree(repoRoot, worktreePath, baseCommit) {
    await git(repoRoot, ["worktree", "add", "--detach", worktreePath, baseCommit], 300_000);
}
export async function cleanWorktreeIncludingIgnored(worktreePath) {
    await git(worktreePath, ["clean", "-ffdx"], 300_000);
    const residue = await ignoredUntrackedPaths(worktreePath);
    if (residue.length)
        throw new Error(`verification worktree retains ignored residue after git clean -ffdx: ${residue.join(", ")}`);
}
export async function applyReviewedPatch(worktreePath, patchPath) {
...
export async function readRepoFile(worktreePath, filePath, baseCommit, requestedOffsetBytes = 0, requestedMaxBytes = MAX_REVIEW_PAGE_BYTES) {
    const relative = normalizeRepoRelative(filePath);
    ...
    if (await pathExists(absolute)) {
        ...
        if (data.length > MAX_REVIEW_FILE_BYTES)
            throw new Error(`${relative} base version exceeds the ${MAX_REVIEW_FILE_BYTES}-byte review limit`);
    }
    if (data.includes(0))
