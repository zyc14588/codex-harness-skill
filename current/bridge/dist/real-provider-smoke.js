...
import { cleanupTask, collectTask, commitTask, createControllerPlan, finalizeControllerPlan, monitorStop, readChangedFile, reviewTask, startTask, taskStatus, verifyTask, } from "./service.js";
...
import { CREDENTIAL_POLICY, OFFICIAL_DEEPSEEK_BASE_URL, readProtectedProviderCredential, } from "./provider-credential.js";
...
async function waitTerminal(taskId, timeoutMs = 600_000) {
...
async function waitProcessesStopped(taskId, timeoutMs = 30_000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const current = payload(await taskStatus(taskId));
        if (current.workerAlive !== true && current.harnessAlive !== true)
            return;
        await sleep(100);
    }
    throw new Error(`real Provider task processes did not stop: ${taskId}`);
}
function safeTaskEvidence(task) {
    return {
        id: task.id,
        model: task.model,
        status: task.status,
...
async function runLeaf(repo, expectedContent, spec) {
    const objective = [
    ...
    if (spec.expectedThinking === "disabled") {
        ...
        assert.deepEqual(requestEvidence.map((entry) => entry.replayRequirementCount), requestEvidence.map((_entry, index) => index), "real Pro request history did not replay 0/1/2... reasoning requirements");
    ...
    const review = payload(await reviewTask(spec.taskId, "approved", [spec.targetPath], `Codex read the complete ${spec.targetPath}, compared it byte-for-byte with README.md, and approved the exact leased change.`));
...
try {
    ...
    if (monitorStarted) {
        ...
        catch { /* task evidence remains authoritative */ }
    }
    try {
        await rm(providerKeyPath, { force: true });
    }
    catch { /* never preserve the broker credential */ }
    ...
    if (priorConfig === undefined)
        ...
        process.env.CODEX_HARNESS_CONFIG = priorConfig;
    if (priorDshHome === undefined)
        delete process.env.DSH_HOME;
    else
        process.env.DSH_HOME = priorDshHome;
    if (priorBaseUrl === undefined)
        delete process.env.DEEPSEEK_BASE_URL;
    else
        process.env.DEEPSEEK_BASE_URL = priorBaseUrl;
    if (priorSmokeBaseUrl === undefined)
        delete process.env.CODEX_REAL_SMOKE_PROVIDER_BASE_URL;
    else
        process.env.CODEX_REAL_SMOKE_PROVIDER_BASE_URL = priorSmokeBaseUrl;
    if (priorProviderKey === undefined)
        delete process.env.DEEPSEEK_API_KEY;
    else
        process.env.DEEPSEEK_API_KEY = priorProviderKey;
    if (priorGithubToken === undefined)
...
//# sourceMappingURL=real-provider-smoke.js.map