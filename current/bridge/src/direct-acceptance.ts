...
function acceptanceTrace(message: string): void {
  if (process.env.CODEX_HARNESS_ACCEPTANCE_TRACE === "1") process.stderr.write(`[acceptance trace] ${message}\n`);
}
...
function payloadArray(value: unknown): Payload[] {
...
async function waitTerminal(taskId: string, timeoutMs = 45_000): Promise<Payload> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const status = payload(await taskStatus(taskId));
    if (!["queued", "running"].includes(String(status.status))) return status;
    await sleep(75);
  }
  throw new Error(`task did not reach a terminal state: ${taskId}`);
}

async function waitProcessesStopped(taskId: string, timeoutMs = 15_000): Promise<Payload> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const status = payload(await taskStatus(taskId));
    if (status.workerAlive !== true && status.harnessAlive !== true) return status;
    await sleep(100);
  }
...
async function reserveLoopbackPort(excluded: ReadonlySet<number> = new Set()): Promise<number> {
...
async function closeServer(server: Server, timeoutMs = 2_000): Promise<void> {
...
async function approveVerifyCommit(taskId: string, message: string): Promise<Payload> {
  ...
  const changed = collected.changedPaths as string[];
...
function outputPathsFromPrompt(prompt: string): string[] {
...
const fakeLlamaCliLauncher = path.join(temp, "pinned-llama-cli");
const fakeManagedServerLauncher = path.join(temp, "pinned-llama-server");
const providerKey = "acceptance-secret-not-persisted";
let providerCalls = 0;
...
const providerServer = http.createServer(async (request, response) => {
  ...
  if (url.pathname.endsWith("/chat/completions")) {
    assert.equal(request.headers.authorization, `Bearer ${providerKey}`, "monitor did not inject the installation-owned Provider credential");
    providerCalls += 1;
...
`);
    ...
    if (requestText.includes("REQUIRED_TOOL_CHOICE_VIOLATION_PROBE")) {
      ...
      const names = (body.tools ?? []).map((tool) => tool.function?.name).filter((name): name is string => typeof name === "string");
...
`);
    ...
    if (requestText.includes("PROVIDER_PROTOCOL_FAILURE_PROBE")) {
      assert.equal(body.model, "deepseek-v4-pro");
      assert.deepEqual(body.thinking, { type: "enabled" });
      assert.equal(body.reasoning_effort, "high");
      assert.equal(body.tool_choice, undefined);
      const names = (body.tools ?? []).map((tool) => tool.function?.name).filter((name): name is string => typeof name === "string");
      assert.ok(names.includes("bash"), `provider protocol failure probe omitted bash: ${JSON.stringify(names)}`);
      response.writeHead(200, { "content-type": "text/event-stream", "cache-control": "no-cache" });
      response.write(`data: ${JSON.stringify({
        ...
        choices: [{
          ...
          delta: {
            tool_calls: [{
              index: 0,
              id: "call-provider-protocol-failure",
              type: "function",
        ...
        usage: { prompt_tokens: 150, completion_tokens: 25, prompt_cache_hit_tokens: 100, prompt_cache_miss_tokens: 50 },
      })}\n\n`);
      response.write("data: [DONE]\n\n");
      return response.end();
    }
    if (requestText.includes("MARKDOWN_SHELL_RECOVERY_PROBE")) {
      ...
      response.write(`data: ${JSON.stringify({ id: `provider-${providerCalls}`, model: body.model, choices: [{ index: 0, delta: { content: second }, finish_reason: "stop" }], usage: { prompt_tokens: 1030, completion_tokens: 116, prompt_cache_hit_tokens: 800, prompt_cache_miss_tokens: 230 } })}\n\n`);
      response.write("data: [DONE]\n\n");
      return response.end();
    }
    ...
    if (body.stream === true) {
      ...
      await sleep(650);
      response.write(`data: ${JSON.stringify({ id: `provider-${providerCalls}`, choices: [{ index: 0, delta: {}, finish_reason: "stop" }], usage: { prompt_tokens: 300, completion_tokens: 12, prompt_cache_hit_tokens: 100, prompt_cache_miss_tokens: 200 } })}\n\n`);
      response.write("data: [DONE]\n\n");
      return response.end();
    }
...
const llamaServer = http.createServer(async (request, response) => {
  ...
  if (url.pathname === "/v1/models") return json(response, 200, { data: [{ id: "acceptance-local-model", object: "model" }] });
  if (url.pathname === "/v1/chat/completions") {
    llamaCalls += 1;
    const body = JSON.parse(await readBody(request)) as { messages?: Array<{ content?: string }> };
    const prompt = body.messages?.[0]?.content ?? "";
    if (prompt.includes("LLAMA_FORCE_TIMEOUT")) await sleep(2_500);
    if (prompt.includes("LLAMA_FORCE_ERROR")) return json(response, 503, { error: "intentional local model failure" });
...
try {
  ...
  await writeFile(configPath, `${JSON.stringify({
    ...
    llamaCpp: {
      ...
      serverBinary: serverIdentity.realpath,
      serverBinarySha256: serverIdentity.sha256,
      serverArgs: [],
      serverAutoStart: false,
      serverStartupTimeoutSeconds: 10,
      cliBinary: cliIdentity.realpath,
      cliBinarySha256: cliIdentity.sha256,
      cliArgs: ["--prompt-file", "{{PROMPT_FILE}}", "--output", "{{OUTPUT_JSON_FILE}}", "-n", "{{MAX_TOKENS}}"],
  ...
  process.env.ACCEPTANCE_PARENT_SECRET = "must-not-reach-child-processes";
  process.env.GITHUB_TOKEN = "must-not-reach-child-processes";
  process.env.DEEPSEEK_API_KEY = providerKey;
  operatorToken = await ensureOperatorToken(await (await import("./config.js")).loadConfig());
  ...
  assert.match(dashboardText, /minlength="12"/, "dashboard must expose the twelve-character operator-password minimum");
  assert.match(dashboardText, /至少 12 个字符/u, "dashboard must explain the twelve-character operator-password minimum");
  ...
  assert.ok(dashboardText.includes("characters<12"), "dashboard password validation lost its twelve-character boundary");
  assert.match(dashboardText, /弱 6 字符兼容模式未启用/u, "dashboard must state that weak compatibility mode is disabled");
  ...
  assert.equal((await fetch(`${dashboardBase}/api/snapshot`)).status, 401, "read-only task data must require operator authentication");
  const rateLimitedAnonymousControl = await fetch(`${dashboardBase}/api/llama/server/start`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: "{}",
  });
  assert.equal(rateLimitedAnonymousControl.status, 429, "repeated unauthenticated process control must be rate-limited");
  assert.ok(Number(rateLimitedAnonymousControl.headers.get("retry-after")) >= 1);
  const rejectedMutation = await fetch(`${dashboardBase}/api/budget-policy`, {
    method: "POST",
    headers: { authorization: `Bearer ${operatorToken}`, "content-type": "application/json", origin: dashboardBase },
  ...
  const invalidCredentialRotation = await fetch(`${dashboardBase}/api/operator-credential`, {
    ...
    body: JSON.stringify({ newToken: "12345678901" }),
  });
  ...
  const nextOperatorToken = "secure-pass-12";
  ...
  const rejectedBinaryReplacement = await fetch(`${dashboardBase}/api/llama/config`, {
    ...
    body: JSON.stringify({ config: { cliBinary: "/bin/sh", cliArgs: ["-c", "env"] } }),
  });
  assert.equal(rejectedBinaryReplacement.status, 400, "runtime control accepted an installation-owned executable replacement");

  const streamPlan = payload(await createControllerPlan({
    repoRoot: repo,
    planId: "stream-plan",
    leaves: [{
  ...
  assert.equal(payloadArray(streamPlan.leaves)[0]?.model, "deepseek-v4-flash", "Harness default model must freeze to Flash before attempt creation");
  await startTask({ planId: "stream-plan", leafId: "stream", taskId: "stream-task" });
  const liveDeadline = Date.now() + 8_000;
  let liveSnapshot: Payload | undefined;
  while (Date.now() < liveDeadline) {
    ...
    if (row && Number(candidate.liveEstimatedCostCny) > 0 && Number(live?.outputTokens ?? 0) > 0) { liveSnapshot = candidate; break; }
    await sleep(100);
  }
  ...
  assert.equal(textualTerminal.minimalMutationPolicyVersion, "minimal-flash-attempt-fixed-v4", JSON.stringify(textualTerminal));
  ...
  await createControllerPlan({
    ...
    leaves: [{
      id: "provider-protocol-fail-fast",
      objective: "PROVIDER_PROTOCOL_FAILURE_TASK",
      executor: "harness",
      complexity: "large",
      harnessMode: "minimal",
      model: "deepseek-v4-pro",
      taskFamily: "provider-protocol-fail-fast-family",
      harnessWritePaths: ["src/harness/provider-protocol-must-not-exist.json"],
      acceptanceCriteria: ["A Pro thinking tool call without real non-empty reasoning fails the attempt immediately"],
      verificationCommands: ["test ! -e src/harness/provider-protocol-must-not-exist.json"],
      runtimeSeconds: 60,
      budget: { maxApiCalls: 3, maxInputTokens: 20_000, maxOutputTokens: 4_000, maxCostCny: 2 },
    }],
  });
  const providerCallsBeforeProtocolFailure = providerCalls;
  const protocolFailureStartedAt = Date.now();
  await startTask({
    planId: "provider-protocol-fail-fast-plan",
    leafId: "provider-protocol-fail-fast",
    taskId: "provider-protocol-fail-fast-task",
  });
  const providerProtocolTerminal = await waitTerminal("provider-protocol-fail-fast-task");
  const providerProtocolElapsedMs = Date.now() - protocolFailureStartedAt;
  assert.equal(providerProtocolTerminal.status, "failed", JSON.stringify(providerProtocolTerminal));
  assert.equal(providerProtocolTerminal.infrastructureFailureKind, "provider_protocol", JSON.stringify(providerProtocolTerminal));
  assert.match(String(providerProtocolTerminal.infrastructureFailureDetails), /omitted non-empty reasoning_content/u);
  assert.equal(providerCalls - providerCallsBeforeProtocolFailure, 1, "provider protocol failure reached the Provider more than once");
  assert.ok(providerProtocolElapsedMs < 8_000, `worker did not fail fast: ${providerProtocolElapsedMs}ms`);
  assert.deepEqual(providerProtocolTerminal.changedPaths, []);
  const providerProtocolUsage = payload(providerProtocolTerminal.usage);
  assert.equal(providerProtocolUsage.apiCalls, 1);
  assert.equal(providerProtocolUsage.failedCalls, 1);
  const providerProtocolAttempts = payloadArray(providerProtocolTerminal.executionAttempts);
  assert.equal(providerProtocolAttempts.length, 1);
  assert.equal(providerProtocolAttempts[0]?.outcome, "failed", JSON.stringify(providerProtocolAttempts));
  assert.match(String(providerProtocolAttempts[0]?.error), /omitted non-empty reasoning_content/u);
  await cleanupTask("provider-protocol-fail-fast-task", true, true);
  ...
  assert.equal(titleIsolationTerminal.minimalMutationPolicyVersion, "minimal-flash-attempt-fixed-v4", JSON.stringify(titleIsolationTerminal));
  ...
  assert.equal(requiredViolationTerminal.minimalMutationPolicyVersion, "minimal-flash-attempt-fixed-v4", JSON.stringify(requiredViolationTerminal));
  ...
  const initialRepairPaths = initialRepairCollect.changedPaths as string[];
  for (const file of initialRepairPaths) await readChangedFile("repair-task", file);
  ...
  await createControllerPlan({
    ...
    leaves: [{
      ...
      budget: { maxApiCalls: 100, maxInputTokens: 600, maxOutputTokens: 1_000, maxCostCny: 100 },
    }],
  });
  ...
  await createControllerPlan({
    ...
    leaves: [{
      ...
      budget: { maxApiCalls: 1, maxInputTokens: 600, maxOutputTokens: 1_000, maxCostCny: 0.000000001 },
    }],
  });
  ...
  while (Date.now() < dynamicDeadline) {
    ...
    if (status.status === "running" && Number(taskUsage.apiCalls) >= 1) { firstCallObserved = true; break; }
    await sleep(75);
  }
  ...
  await dashboardMutation(dashboardBase, csrf, "/api/budget-overrides", "POST", {
    ...
    budget: { maxApiCalls: 1, maxInputTokens: 800, maxOutputTokens: 1_000, maxCostCny: 0.000000001, maxCostUsd: 0.000000001 },
  ...
  assert.equal(payload(dynamicTerminal.budget).maxInputTokens, 800);
  assert.equal(payload(dynamicTerminal.frozenBudget).maxInputTokens, 600);
  ...
  async function runFallbackCase(planId: string, leafId: string, taskId: string, objective: string, output: string, expectedText: string): Promise<void> {
  ...
  await runFallbackCase(
    "llama-error-fallback-plan", "local-error", "llama-error-fallback-task",
    "LLAMA_FALLBACK_TASK LLAMA_FORCE_ERROR", "src/local/fallback.txt", "Harness deepseek-v4-flash fallback",
  );
  await runFallbackCase(
    "llama-omission-plan", "local-omit", "llama-omission-task",
    "LLAMA_OMIT_OUTPUT", "src/local/omitted.txt", "deepseek-v4-flash fallback after invalid local output",
  );
  await runFallbackCase(
    "llama-timeout-plan", "local-timeout", "llama-timeout-task",
    "LLAMA_TIMEOUT_TASK LLAMA_FORCE_TIMEOUT", "src/local/timeout.txt", "deepseek-v4-flash fallback after local timeout",
  );
  ...
  while (Date.now() < overlapDeadline) {
    ...
    if (a.status === "running" && b.status === "running") { simultaneous = true; break; }
    await sleep(50);
  }
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    providerProtocolAttemptFailFast: "PASS",
  ...
  delete process.env.ACCEPTANCE_PARENT_SECRET;
  delete process.env.GITHUB_TOKEN;
  delete process.env.DEEPSEEK_API_KEY;
  acceptanceTrace("finally: monitor stop begin");
  try { await monitorStop(); } catch { /* best effort */ }
  ...
  if (process.env.KEEP_CODEX_HARNESS_ACCEPTANCE !== "1") await rm(temp, { recursive: true, force: true });
