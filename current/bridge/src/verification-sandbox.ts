...
async function gitCommonDirectory(worktree: string): Promise<string> {
  ...
  if (result.code !== 0 || !result.stdout.trim()) throw new Error(`cannot resolve verification Git common directory: ${result.stderr.trim()}`);
  return await realpath(result.stdout.trim());
}

export async function runVerificationSandboxCommand(
  config: BridgeConfig,
  worktreeInput: string,
  shellCommand: string,
  timeoutSeconds: number,
  profile: FrozenHostResourceProfile,
): Promise<VerificationSandboxResult> {
  if (process.platform !== "linux") throw new Error("authoritative verification requires Linux Bubblewrap isolation");
  ...
  if (isWithin(gitCommon, worktree)) throw new Error("verification requires a linked worktree with a separately mountable Git common directory");
  ...
  for (const directory of destinationDirectories(destinations)) args.push("--dir", directory);
  args.push(
    "--ro-bind", "/usr", "/usr",
    "--symlink", "usr/bin", "/bin",
  ...
  if (externalNode) args.push("--ro-bind", nodeExecutable, nodeExecutable);
  const nodePath = path.dirname(nodeExecutable);
  args.push(
    "--setenv", "PATH", `${nodePath}:/usr/local/bin:/usr/bin:/bin`,
    "--setenv", "HOME", "/tmp",
    "--setenv", "USER", "codex-harness-verify",
    "--setenv", "LOGNAME", "codex-harness-verify",
    ...
    "--chdir", worktree,
    "--", "/bin/bash", "--noprofile", "--norc", "-lc", shellCommand,
  );
