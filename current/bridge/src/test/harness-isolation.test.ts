...
async function processIdsContaining(marker: string): Promise<number[]> {
  const values: number[] = [];
  for (const entry of await readdir("/proc")) {
    if (!/^\d+$/u.test(entry)) continue;
    try {
      const commandLine = await readFile(`/proc/${entry}/cmdline`, "utf8");
      if (commandLine.includes(marker)) values.push(Number(entry));
    } catch { /* process exited while scanning */ }
  }
  return values;
}

async function waitUntil(label: string, predicate: () => Promise<boolean>, timeoutMs = 5_000): Promise<void> {
...
`, { mode: 0o700 });
    ...
    const config = {
      ...
      harnessIsolation: {
        ...
        relayPort: 49_152,
        rejectEnvFiles: true as const,
        resourceProfile: {
          ...base.harnessIsolation.resourceProfile,
          enforcement: "audit_only" as const,
    ...
    assert.equal(await socketRaw(
      monitorSocketPath(config),
      adapterRoute,
      proxyToken,
      Buffer.from(JSON.stringify({ taskId, toolNames: [] })),
    ), 403, "Provider capability was accepted by the Adapter-only state route");
    assert.equal(await socketRaw(
      monitorSocketPath(config),
      adapterRoute,
      toolToken,
      Buffer.from(JSON.stringify({ taskId, toolNames: [] })),
    ), 403, "tool capability was accepted by the Adapter-only state route");
    ...
    const toolReport = JSON.parse(await readFile(path.join(worktree, "tool-isolation-report.json"), "utf8")) as Record<string, unknown>;
    ...
    const monitorClosed = new Promise<void>((resolve) => monitor!.once("close", () => resolve()));
