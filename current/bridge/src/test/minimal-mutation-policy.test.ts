...
test("forces a first mutation tool call for a diff-free minimal Flash leaf", () => {
  const result = applyMinimalMutationPolicy(task, request, [], "deepseek-v4-flash", "primary_mutation");
  ...
  assert.equal(MINIMAL_MUTATION_POLICY_VERSION, "minimal-flash-attempt-fixed-v4");
});

test("keeps Minimal Flash disabled after a real diff exists within the same attempt", () => {
  const result = applyMinimalMutationPolicy(task, request, ["probe.json"], "deepseek-v4-flash", "mutation_followup");
  assert.equal(result.applied, false);
  assert.notEqual(result.body, request);
  assert.deepEqual(result.body.thinking, { type: "disabled" });
  assert.equal("reasoning_effort" in result.body, false);
  assert.equal(result.body.tool_choice, undefined);
  assert.deepEqual(requestToolNames(result.body), requestToolNames(request));
});

test("does not force Pro, standard, or non-mutating tasks", () => {
  assert.equal(applyMinimalMutationPolicy(task, request, [], "deepseek-v4-pro", "primary_mutation").applied, false);
  assert.equal(applyMinimalMutationPolicy({ ...task, harnessMode: "standard" }, request, [], "deepseek-v4-flash", "primary_mutation").applied, false);
  ...
  assert.equal(applyMinimalMutationPolicy(task, analysis, [], "deepseek-v4-flash", "non_mutating_agent_request").applied, false);
});
...
test("bypasses the Harness first-prompt title request even though it embeds the bounded contract", () => {
  ...
  const result = applyMinimalMutationPolicy(task, titleRequest, [], "deepseek-v4-flash", "session_title_auxiliary");
  assert.equal(result.applied, false);
  assert.equal(result.reason, undefined);
...
test("still fails a primary request that advertises tools but omits every core mutation tool", () => {
  ...
  const result = applyMinimalMutationPolicy(task, withoutCore, [], "deepseek-v4-flash", "primary_mutation");
  assert.equal(result.applied, false);
...
test("fails an unrecognized tool-less primary mutation request instead of silently bypassing it", () => {
  ...
  const result = applyMinimalMutationPolicy(task, noTools, [], "deepseek-v4-flash", "primary_mutation");
  assert.equal(result.applied, false);
