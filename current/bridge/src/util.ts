...
export function safeTaskId(input?: string): string {
  if (input) {
...
export function validateLeasePattern(input: string): string {
  ...
  if (normalized.endsWith("/**")) {
    ...
    if (!prefix || prefix.includes("*") || prefix.includes("?")) throw new Error(`invalid directory lease: ${input}`);
    return `${prefix}/**`;
  }
...
export function leaseMatches(lease: string, filePath: string): boolean {
...
function leaseRoot(lease: string): string {
  if (lease === "**") return "";
...
export function leasesOverlap(a: string, b: string): boolean {
  ...
  if (!aDir && !bDir) return ar === br;
  if (aDir && bDir) return ar === br || ar.startsWith(`${br}/`) || br.startsWith(`${ar}/`);
  if (aDir) return br === ar || br.startsWith(`${ar}/`);
  return ar === br || ar.startsWith(`${br}/`);
}

export function assertDisjointLeases(harness: string[], codex: string[]): void {
  for (const h of harness) {
    for (const c of codex) {
      if (leasesOverlap(h, c)) throw new Error(`write-scope overlap is forbidden: Harness '${h}' vs Codex '${c}'`);
    }
  }
}
...
export async function tailText(target: string, maxChars: number): Promise<string> {
...
export function boundedText(value: string, field: string, maxChars: number): string {
  const trimmed = value.trim();
  if (!trimmed) throw new Error(`${field} must not be empty`);
  if (trimmed.includes("\0")) throw new Error(`${field} must not contain NUL characters`);
  if (trimmed.length > maxChars) throw new Error(`${field} exceeds ${maxChars} characters`);
  return trimmed;
}

export function boundedStringList(values: string[], field: string, maxItems: number, maxCharsPerItem: number): string[] {
...
async function updateHashWithFile(hash: Hash, target: string): Promise<void> {
  for await (const chunk of createReadStream(target)) hash.update(chunk as Buffer);
}

export async function sha256PathTree(target: string): Promise<string> {
...
export async function runProcess(
  ...
  return await new Promise<ProcessResult>((resolve, reject) => {
    ...
    let reportedSignal: NodeJS.Signals | null | undefined;
    let settled = false;
    ...
    const signalProcess = async (signal: NodeJS.Signals): Promise<void> => {
      try {
        if (useProcessGroup && identityPromise) await signalVerifiedProcessGroup(await identityPromise, signal);
    ...
    identityPromise?.catch((error) => {
      try { child.kill("SIGKILL"); } catch { /* failed supervisor is already gone */ }
      fail(error);
    });
    if (useProcessGroup) {
    ...
    if (!useProcessGroup && options.input !== undefined) child.stdin?.end(options.input);
...
export function jsonToolResult(value: unknown, isError = false): { content: Array<{ type: "text"; text: string }>; isError?: boolean } {
