...
const executableDigestCache = new Map<string, DigestCacheEntry>();
...
function digestMetadata(info: Awaited<ReturnType<typeof stat>>): string {
  return [info.dev, info.ino, info.size, info.mode, info.uid, info.gid, info.mtimeMs, info.ctimeMs].join(":");
}

async function sha256StableExecutable(target: string): Promise<string> {
  ...
  if (digestMetadata(after) !== metadata) throw new Error(`executable changed while hashing: ${target}`);
  executableDigestCache.set(target, { metadata, sha256 });
  return sha256;
}

function parseProcStat(text: string): ProcStatIdentity {
  const close = text.lastIndexOf(")");
  if (close < 0) throw new Error("/proc stat has no closing command delimiter");
...
export async function captureProcessIdentity(pid: number): Promise<ProcessIdentity> {
  if (process.platform !== "linux") throw new Error("strong process identity requires Linux /proc");
 ...
 */
export async function captureSettledProcessIdentity(pid: number, timeoutMs = 2_000, stableMs = 75): Promise<ProcessIdentity> {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (await processIdentityMatches(identity)) return identity;
...
export async function processIdentityMatches(identity: ProcessIdentity | undefined): Promise<boolean> {
...
export async function assertProcessIdentity(identity: ProcessIdentity, label: string): Promise<void> {
 ...
 */
export async function signalVerifiedProcessGroup(identity: ProcessIdentity, signal: NodeJS.Signals): Promise<boolean> {
...
export async function sha256Executable(target: string): Promise<{ realpath: string; sha256: string }> {
  const canonical = await realpath(target);
  const info = await stat(canonical);
  if (!info.isFile() || (info.mode & 0o111) === 0) throw new Error(`allowlisted executable must be an executable regular file: ${canonical}`);
  if ((info.mode & 0o022) !== 0) throw new Error(`allowlisted executable must not be group/world writable: ${canonical}`);
