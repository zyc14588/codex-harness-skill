...
import {
  ...
  binaryPatch,
  changedPaths,
  cleanWorktreeIncludingIgnored,
  commitLog,
  createDetachedVerificationWorktree,
  createCommit,
  createWorktree,
  deleteBranch,
...
export function governedHarnessMode(
  executor: "harness" | "llama_cpp",
  requested: HarnessExecutionMode | undefined,
  label: string,
): HarnessExecutionMode {
...
async function snapshotFingerprint(worktreePath: string, baseCommit: string): Promise<string> {
...
export async function doctor(probeHarness: boolean): Promise<unknown> {
  ...
  const resourceProbes: Record<string, unknown> = {};
  ...
  for (const id of RESOURCE_PROFILE_IDS) {
    ...
    if (!probe.controlledUseAllowed) allResourceProfilesControlled = false;
  }
  const resourceProbe = resourceProbes.authoritative_verification as Awaited<ReturnType<typeof probeHostResourceProfile>>;
...
function mergeReadRanges(ranges: Array<{ offsetBytes: number; returnedBytes: number }>): Array<{ offsetBytes: number; returnedBytes: number }> {
  const sorted = ranges
  ...
  const merged: Array<{ offsetBytes: number; returnedBytes: number }> = [];
...
export async function readChangedFile(
  taskId: string,
  filePath: string,
  offsetBytes = 0,
  maxBytes = MAX_REVIEW_PAGE_BYTES,
): Promise<unknown> {
  const config = await loadConfig();
  const id = safeTaskId(taskId);
  const initial = await loadTask(config, id);
  return await withWorktreeLock(config, initial.worktreePath, async () => {
    ...
    const receipt: ChangedFileReadReceipt = {
      schemaVersion: 1,
      path: normalized,
...
export async function verifyTask(taskId: string, commands?: string[], timeoutSeconds = 1800): Promise<unknown> {
  ...
  return await withWorktreeLock(config, initial.worktreePath, async () => {
    ...
    let finalUnsafeSymlinks: string[] = [];
    let finalUnsafeGitlinks: string[] = [];
    let finalStaged: string[] = [];
    ...
    try {
      ...
      if (!cleanStart) throw new Error("fresh verification worktree is not a clean checkout of baseCommit");
      if (reviewedPatch.length > 0) await applyReviewedPatch(verificationDirectory, reviewedPatchPath);
      ...
      for (const command of selected) {
        const result = await runVerificationSandboxCommand(
          config,
    ...
    const passed = cleanStart && verificationWorktreeRemoved && verificationHeadStable
      ...
      && results.every((item) => {
        const sandbox = item.sandbox as Record<string, unknown> | undefined;
    ...
    task.verificationResourceProbe = verificationResourceProbe as unknown as Record<string, unknown>;
    ...
    if (!fingerprintStable) task.error = "fresh authoritative verification or the agent tree differs from the reviewed snapshot; re-review before adoption";
