...
export async function inspectMinimalPresetBrokerComposition(
  presetDirectory: string,
  trustedTemplatePath = fileURLToPath(new URL("../../harness/minimal/preset/agent.cordis.yml.in", import.meta.url)),
  profile = "codex-minimal-headless",
): Promise<MinimalPresetBrokerInspection> {
  ...
  catch { errors.push(`trusted minimal preset template is unreadable: ${trustedTemplatePath}`); }
  try { configured = await readFile(presetPath, "utf8"); }
  ...
  if (configured !== undefined) {
    ...
    if (brokerEntries.length !== 1) errors.push("managed minimal preset must contain exactly one bridge-brokered-tools entry");
...
export async function prepareHarnessSandbox(
  ...
  if (!task.adapterToken || !/^[a-f0-9]{64}$/.test(task.adapterToken)) throw new Error("Harness task has no valid Adapter-only state credential");
  if (!task.toolToken || !/^[a-f0-9]{64}$/.test(task.toolToken)) throw new Error("Harness task has no valid brokered-tool credential");
