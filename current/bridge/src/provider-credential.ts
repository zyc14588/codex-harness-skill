...
export const REVOCATION_ACCEPTED_STATUSES = Object.freeze([401, 403] as const);
...
export function credentialFingerprintSha256(rawBytes: Uint8Array): string {
  assert.ok(rawBytes.byteLength >= 24, "real Provider API key is malformed");
  return createHash("sha256").update(rawBytes).digest("hex");
}

export async function readProtectedProviderCredential(source: string): Promise<ProtectedProviderCredential> {
...
export function isCredentialRevocationStatus(httpStatus: number): httpStatus is 401 | 403 {
  return REVOCATION_ACCEPTED_STATUSES.includes(httpStatus as 401 | 403);
}

export async function probeOfficialDeepSeekCredential(apiKey: string, timeoutMs: number): Promise<number> {
...
export async function waitForCredentialRevocation(
  options: WaitForCredentialRevocationOptions,
): Promise<CredentialRevocationEvidence> {
  ...
  const delay = options.delay ?? (async (milliseconds: number) => {
  ...
  while (now() < deadlineMs) {
    ...
    try {
      ...
      if (Number.isInteger(observed) && observed >= 100 && observed <= 599) httpStatus = observed;
    } catch {
      httpStatus = 0;
    }
    attempt += 1;
    ...
    if (afterProbeMs >= deadlineMs) break;
