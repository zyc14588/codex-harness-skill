...
export class LlamaExecutionError extends Error {
  ...
  constructor(code: LlamaFailureCode, message: string, fallbackEligible = code !== "budget" && code !== "security" && code !== "resource") {
    super(message);
...
async function readManagedState(config: BridgeConfig): Promise<ManagedLlamaServerState | undefined> {
  ...
  try {
    const state = await readJson<ManagedLlamaServerState>(target);
    return state.schemaVersion === 2 && state.identity !== undefined ? state : undefined;
  } catch { return undefined; }
}

function localModelEnvironment(config: BridgeConfig): NodeJS.ProcessEnv {
  ...
  if (key !== undefined) env.LLAMA_CPP_API_KEY = key;
  if (process.env.LC_ALL !== undefined) env.LC_ALL = process.env.LC_ALL;
  if (process.env.SSL_CERT_FILE !== undefined) env.SSL_CERT_FILE = process.env.SSL_CERT_FILE;
  return env;
}

async function pinnedExecutable(command: string, expectedSha256: string | undefined, label: string): Promise<string> {
  if (!path.isAbsolute(command)) throw new LlamaExecutionError("security", `${label} must be an absolute allowlisted path`, false);
  if (!expectedSha256) throw new LlamaExecutionError("security", `${label} has no SHA-256 allowlist pin`, false);
...
export async function startManagedLlamaServer(config: BridgeConfig): Promise<Record<string, unknown>> {
  return await withNamedLock(config, "llama-managed-server", 30_000, async () => {
    ...
    if (await processIdentityMatches(existing?.identity)) return await managedLlamaServerStatus(config, true);
    ...
    try {
      ...
      if (identity.processGroupId !== identity.pid) throw new Error("managed llama-server did not become its process-group leader");
      ...
      while (Date.now() < deadline) {
        if (!await processIdentityMatches(identity)) throw new Error(`managed llama-server exited during startup; inspect ${state.logPath}`);
...
export async function stopManagedLlamaServer(config: BridgeConfig): Promise<Record<string, unknown>> {
  return await withNamedLock(config, "llama-managed-server", 30_000, async () => {
    ...
    while (Date.now() < deadline && await processIdentityMatches(state.identity)) await sleep(50);
...
async function runCli(config: BridgeConfig, settings: LlamaCppConfig, task: TaskRecord, prompt: string, maxOutput: number): Promise<{ content: string; inputTokens: number; outputTokens: number; latencyMs: number }> {
  ...
  if (settings.cliArgs.some((arg) => arg.includes("{{PROMPT}}"))) throw new LlamaExecutionError("security", "llama.cpp prompt text is forbidden in argv", false);
  const containsPromptFile = settings.cliArgs.some((arg) => arg.includes("{{PROMPT_FILE}}"));
  if (!containsPromptFile) throw new LlamaExecutionError("process", "llama.cpp cliArgs must include {{PROMPT_FILE}}");
  const args = commandArgs(settings, settings.cliArgs, variables);
  const executable = await pinnedExecutable(settings.cliBinary, settings.cliBinarySha256, "llama.cpp cliBinary");
  if (!task.resourceProfile) throw new LlamaExecutionError("resource", "llama.cpp task has no frozen DEC-003 resource profile", false);
  ...
  try {
    ...
    if (result.stderr) await writeFile(task.stderrPath, `[llama-cli ${commandDisplay(executable, args)}]\n${result.stderr}\n`, { encoding: "utf8", mode: 0o600, flag: "a" });
    if (result.timedOut) throw new LlamaExecutionError("timeout", `llama-cli exceeded ${Math.min(task.runtimeSeconds, settings.requestTimeoutSeconds, task.resourceProfile.commandTimeoutSeconds)} seconds`);
