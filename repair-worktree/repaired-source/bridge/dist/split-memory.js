...
import { failureAttribution, infrastructureAnomalyLabels } from "./infrastructure-failure.js";
...
export async function loadSplitMemoryProfile(config, repoRoot, memoryKey) {
    return (await loadSplitMemory(config, repoRoot, memoryKey)).profile;
}
export async function adviseSplit(config, repoRoot, descriptor) {
    ...
    const complexity = recommendedComplexity(descriptor.proposedComplexity, profile, config.controller.splitMemory.minSamplesForEnforcement);
    const defaultInput = descriptor.defaultBudget.maxInputTokens;
    const defaultOutput = descriptor.defaultBudget.maxOutputTokens;
    // An infrastructure-only profile is diagnostic state, not task-shape
    // evidence. Until at least one learnable execution exists, preserve the
    // caller's current proposal exactly even when the infrastructure event was
    // recorded from a task with different token gates.
    const rememberedInput = sampleCount > 0
        ? profile?.recommendedMaxInputTokens ?? defaultInput
        : defaultInput;
    const rememberedOutput = sampleCount > 0
        ? profile?.recommendedMaxOutputTokens ?? defaultOutput
        : defaultOutput;
    const rationale = [];
    if (!config.controller.splitMemory.enabled)
        ...
        if (complexity !== descriptor.proposedComplexity)
            rationale.push(`historical outcomes recommend ${complexity} instead of ${descriptor.proposedComplexity}`);
        if (profile.tokenGateExceededCount > 0)
            rationale.push(`${profile.tokenGateExceededCount} token-gate overrun(s) require smaller leaves and observed-usage safety margin`);
        if (profile.repairCount > 0 || profile.verificationFailureCount > 0)
            rationale.push("repair or verification failures reduce the next leaf size");
        if (profile.infrastructureFailureCount > 0)
            rationale.push(`${profile.infrastructureFailureCount} infrastructure failure(s) were recorded separately and did not shrink leaf sizing`);
    }
    return {
        repoKey,
        memoryKey,
...
function ema(previous, current, alpha = 0.25) {
...
function infrastructureAnomalies(task) {
    return infrastructureAnomalyLabels(task).map((kind) => `${kind}_failure`);
}
function executionAnomalies(task, usage) {
    ...
    if (infrastructureAnomalies(task).length > 0)
        return [];
    const anomalies = new Set();
    const input = usage.inputTokens + usage.estimatedInputTokens;
    const output = usage.outputTokens + usage.estimatedOutputTokens;
    if (input > task.budget.maxInputTokens || /input token budget/i.test(task.error ?? ""))
        anomalies.add("input_token_gate_exceeded");
    if (output > task.budget.maxOutputTokens || /output token budget/i.test(task.error ?? ""))
        anomalies.add("output_token_gate_exceeded");
    if (taskTimedOut(task))
        anomalies.add("timeout");
    if (task.status === "failed")
        anomalies.add("execution_failed");
    if (task.status === "scope_violation")
        anomalies.add("scope_violation");
    if (task.status === "orphaned")
        anomalies.add("orphaned");
    if (task.fallbackUsed)
        anomalies.add("fallback_used");
    if (task.parentTaskId || task.mode === "repair")
        anomalies.add("repair_required");
    return [...anomalies].sort();
}
function stageAnomalies(task, stage, execution) {
    if (infrastructureAnomalies(task).length > 0)
        return [];
    if (stage === "execution")
        return [...execution];
    const anomalies = new Set();
    if (stage === "review" && task.reviewDecision === "revise")
        anomalies.add("review_revise");
    if (stage === "review" && task.reviewDecision === "rejected")
        anomalies.add("review_rejected");
    if (stage === "verification" && task.verificationPassed === false)
        anomalies.add("verification_failed");
    return [...anomalies].sort();
}
function eventStatus(task, stage) {
    if (stage === "review")
        return task.reviewDecision ?? "not_reviewed";
    if (stage === "verification")
        return task.verificationPassed === true ? "passed" : task.verificationPassed === false ? "failed" : "not_verified";
    return task.status;
}
function initialProfile(event, now) {
    return {
...
function applyOutcome(config, profile, event) {
    ...
    if (event.attribution === "infrastructure") {
        if (event.stage === "execution")
            next.infrastructureFailureCount += 1;
        return next;
    }
    ...
    if (severe)
        scale *= 1 - memory.anomalyPenalty;
    else if (moderate)
        scale *= 1 - memory.anomalyPenalty / 2;
    else if (successful && Math.max(event.inputRatio, event.outputRatio, event.runtimeRatio) <= 0.75)
        scale *= 1 + memory.successGrowth;
    ...
    if (next.sampleCount >= memory.minSamplesForEnforcement) {
        if (next.recommendedLeafScale < 0.5)
            next.recommendedComplexity = downgrade(event.chosenComplexity, 2);
        else if (next.recommendedLeafScale < 0.82)
            next.recommendedComplexity = downgrade(event.chosenComplexity, 1);
        else if (next.recommendedLeafScale > 1.2 && next.complexLeafConfidence >= 0.75)
            next.recommendedComplexity = upgrade(event.chosenComplexity, 1);
        else
            next.recommendedComplexity = event.chosenComplexity;
    }
    if (learnableExecution) {
...
export async function recordTaskSplitOutcome(config, task, stage) {
    if (!config.controller.splitMemory.enabled)
        return undefined;
    ...
    return await withNamedLock(config, `split-memory:${repoKey}:${task.splitDecision.memoryKey}`, 30_000, async () => {
        if (await pathExists(marker))
            return await loadSplitMemoryProfile(config, task.repoRoot, task.splitDecision.memoryKey);
        ...
        const attribution = failureAttribution(task.infrastructureFailureKind)
            ?? (anomalies.length > 0 ? "task_shape" : "neutral");
        const event = {
            schemaVersion: SPLIT_MEMORY_SCHEMA_VERSION,
            id: randomUUID(),
            at: nowIso(),
        ...
        if (await pathExists(target)) {
            try {
                ...
                if (raw.schemaVersion === SPLIT_MEMORY_SCHEMA_VERSION)
                    existing = raw;
                else {
...
export async function listSplitMemoryProfiles(config, repoRoot) {
    const root = repoRoot
...
//# sourceMappingURL=split-memory.js.map