...
function stableCallId(seed, index) {
    return `call_bridge_${createHash("sha256").update(seed).update(`\n${index}`).digest("hex").slice(0, 24)}`;
}
function parseParameterValue(raw, stringAttribute) {
    const decoded = decodeXml(raw);
    if (stringAttribute?.toLowerCase() === "true")
        return decoded;
    if (stringAttribute?.toLowerCase() === "false")
...
function previousMatch(re, text, before) {
    re.lastIndex = 0;
    let found;
...
function nextMatch(re, text, after) {
    re.lastIndex = after;
    return re.exec(text) ?? undefined;
}
export function allowedToolNamesFromRequest(body) {
    const names = new Set();
...
export function parseDsmlToolCalls(text, allowedNames) {
    ...
    for (let match = INVOKE_RE.exec(text); match; match = INVOKE_RE.exec(text)) {
        ...
        if (!name)
            return { calls: [], remainingText: text, markerFound: true, failure: "DSML invoke is missing a tool name" };
        ...
        for (let parameter = PARAM_RE.exec(body); parameter; parameter = PARAM_RE.exec(body)) {
            ...
            if (!parameterName)
                return { calls: [], remainingText: text, markerFound: true, failure: `DSML tool ${name} has a parameter without a name` };
...
function taskContractText(body) {
 ...
 */
export function isMutatingBoundedLeafRequest(body) {
    const contract = taskContractText(body);
    if (!contract || !MUTATING_MODE_RE.test(contract))
        return false;
...
export function parseMarkdownShellToolCall(text, allowedNames, requestBody) {
...
function argumentObject(value, name) {
    if (value && typeof value === "object" && !Array.isArray(value))
        return { value: value };
    if (typeof value !== "string")
        return { failure: `textual tool call ${name} arguments must be a JSON object or string` };
    const trimmed = value.trim();
    if (trimmed.length === 0)
        return { failure: `textual tool call ${name} arguments are empty` };
    try {
...
function callFromRecord(record, allowedNames, seed, index, forcedName) {
    ...
    if (!name)
        return { failure: "textual tool call is missing a tool name" };
    if (!allowedNames.has(name))
        return { failure: `textual response requested an undisclosed tool: ${name}` };
    ...
    if (!parsedArguments.value)
        return { failure: parsedArguments.failure ?? `textual tool call ${name} has invalid arguments` };
...
function looksLikeCallRecord(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return false;
    const record = value;
    if (Array.isArray(record.tool_calls) || Array.isArray(record.calls))
        return true;
...
function callsFromJsonValue(value, allowedNames, seed, forcedName) {
    let entries;
    if (Array.isArray(value))
        entries = value;
        ...
        if (Array.isArray(record.tool_calls))
            entries = record.tool_calls;
        else if (Array.isArray(record.calls))
            entries = record.calls;
        else
            entries = [record];
    }
    else
        return { failure: "textual tool-call payload must be a JSON object or array" };
    if (entries.length === 0)
        return { failure: "textual tool-call payload is empty" };
    if (entries.length > MAX_RECOVERED_CALLS)
        return { failure: `textual response contains more than ${MAX_RECOVERED_CALLS} tool calls` };
    const calls = [];
    for (const [index, entry] of entries.entries()) {
        if (!entry || typeof entry !== "object" || Array.isArray(entry))
            return { failure: `textual tool call ${index} is not a JSON object` };
        const parsed = callFromRecord(entry, allowedNames, seed, index, forcedName);
        if (!parsed.call)
            return { failure: parsed.failure ?? `textual tool call ${index} is invalid` };
        calls.push(parsed.call);
    }
    return { calls };
}
function parseJsonToolPayload(raw, allowedNames, seed, forcedName) {
    let value;
    try {
        value = JSON.parse(raw.trim());
    }
    catch (error) {
        return { failure: `textual tool-call JSON is invalid: ${error instanceof Error ? error.message : String(error)}` };
    }
 ...
 */
export function parseTextualToolCallEnvelope(text, allowedNames, requestBody) {
    if (!isMutatingBoundedLeafRequest(requestBody))
        return { calls: [], remainingText: text, markerFound: false };
        ...
        if (wrappers.length > 0) {
            ...
            for (const [index, wrapper] of wrappers.entries()) {
                ...
                for (const call of parsed.calls)
                    calls.push({ ...call, id: call.id || stableCallId(wrapper[0], index) });
            }
            return { calls, remainingText: "", markerFound: true };
        }
    ...
    if (single) {
        ...
        if (parsed.calls)
            return { calls: parsed.calls, remainingText: "", markerFound: true };
        if (forcedName && (forcedName === "bash" || forcedName === "pwsh") && inner.length > 0) {
            ...
            if (direct.call)
                return { calls: [direct.call], remainingText: "", markerFound: true };
        }
        return { calls: [], remainingText: text, markerFound: true, failure: parsed.failure ?? "textual tool-call payload is invalid" };
    }
    ...
    if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
        ...
        const markerFound = Boolean(value && typeof value === "object" && (Array.isArray(value)
            ? value.length > 0 && value.every(looksLikeCallRecord)
            : looksLikeCallRecord(value)
                || Object.keys(value).some((key) => ["tool_calls", "calls"].includes(key))));
        if (markerFound) {
    ...
    if (bracket) {
        ...
        if (!allowedNames.has(name))
            return { calls: [], remainingText: text, markerFound: true, failure: `textual response requested an undisclosed tool: ${name}` };
...
function collectSse(buffer) {
    const payloads = ssePayloads(buffer);
    const envelope = { base: {}, reasoning: "", content: "", toolCalls: new Map(), structuredNeedsNormalization: false };
    for (const payload of payloads) {
        ...
        for (const rawChoice of choices) {
            ...
            for (const rawCall of toolCalls) {
                ...
                if (call.id !== undefined && id === undefined)
                    envelope.structuredNeedsNormalization = true;
                if (id !== undefined)
                    current.id = id;
...
function synthesizedSse(envelope, calls, remainingText) {
    const chunks = [];
...
function normalizedStructuredCalls(envelope, allowedNames) {
    ...
    for (const [index, call] of [...envelope.toolCalls.entries()].sort((left, right) => left[0] - right[0])) {
        ...
        if (!name)
            return { failure: `structured tool call ${index} has no stable name` };
        if (!allowedNames.has(name))
            return { failure: `structured response requested an undisclosed tool: ${name}` };
        let parsed;
        try {
            parsed = JSON.parse(call.arguments || "{}");
        }
        catch (error) {
            return { failure: `structured tool call ${name} has invalid JSON arguments: ${error instanceof Error ? error.message : String(error)}` };
        }
        if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
            return { failure: `structured tool call ${name} arguments must be a JSON object` };
        calls.push({ name, arguments: JSON.stringify(parsed), id: call.id ?? stableCallId(`${name}\n${call.arguments}`, index) });
    }
    return { calls };
}
function recoveredSse(body, envelope, parsed, kind) {
    if (parsed.failure) {
        return {
            body,
...
function transformSse(body, allowedNames, requestBody) {
    ...
    if (dsml.markerFound)
        return recoveredSse(body, envelope, dsml, "dsml_content_to_tool_calls");
    const markdown = parseMarkdownShellToolCall(envelope.content, allowedNames, requestBody);
    if (markdown.markerFound)
        return recoveredSse(body, envelope, markdown, "markdown_shell_fence_to_tool_calls");
    const textual = parseTextualToolCallEnvelope(envelope.content, allowedNames, requestBody);
    if (textual.markerFound)
        return recoveredSse(body, envelope, textual, "text_tool_call_envelope_to_tool_calls");
    return { body, contentType: "text/event-stream; charset=utf-8", changed: false, recoveryKinds: [], recoveredToolNames: [] };
}
function applyJsonCalls(choice, message, parsed, kind, recoveredNames, kinds) {
    if (parsed.failure)
...
function transformJson(body, allowedNames, requestBody) {
    let root;
    try {
    ...
    for (const rawChoice of choices) {
        ...
        if (Array.isArray(existing) && existing.length > 0) {
            const envelope = { base: {}, reasoning: "", content: typeof message.content === "string" ? message.content : "", toolCalls: new Map(), structuredNeedsNormalization: false };
            existing.forEach((rawCall, index) => {
                if (!rawCall || typeof rawCall !== "object" || Array.isArray(rawCall))
                    return;
                const call = rawCall;
                const fnRaw = call.function;
                const fn = fnRaw && typeof fnRaw === "object" && !Array.isArray(fnRaw) ? fnRaw : {};
                const stableId = nonEmpty(call.id);
                const stableName = nonEmpty(fn.name);
                if (stableId === undefined || stableName === undefined)
                    envelope.structuredNeedsNormalization = true;
            ...
            if (normalized.failure)
                return { body, contentType: "application/json; charset=utf-8", changed: false, recoveryKinds: [], recoveredToolNames: [], failure: normalized.failure };
        ...
        if (failure)
            return { body, contentType: "application/json; charset=utf-8", changed: false, recoveryKinds: [], recoveredToolNames: [], failure };
        changed = true;
    }
    return {
...
export function transformProviderToolCalls(contentType, body, requestBody) {
    ...
    if (contentType.includes("json"))
        return transformJson(body, allowedNames, requestBody);
