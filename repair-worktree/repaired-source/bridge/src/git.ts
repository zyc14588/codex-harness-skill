...
function patchPathLabel(file: string): string {
...
async function renderUntrackedFilePatch(worktreePath: string, file: string): Promise<string> {
  const relative = normalizeRepoRelative(file);
  const absolute = path.resolve(worktreePath, relative);
  const root = path.resolve(await realpath(worktreePath));
  if (!isWithin(absolute, root)) throw new Error(`untracked path escapes worktree: ${relative}`);
  ...
  if (!info.isFile()) throw new Error(`untracked path is not a regular file or symlink: ${relative}`);
  if (info.size > MAX_PATCH_CHARS) throw new Error(`untracked file ${relative} exceeds patch safety limit`);
  ...
  if (text === undefined) return [...header, "content-encoding binary-sha256-only"].join("\n");
