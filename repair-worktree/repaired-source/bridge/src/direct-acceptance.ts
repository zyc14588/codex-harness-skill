...
try {
  ...
  assert.equal((await fetch(`${dashboardBase}/api/llama/server/start`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: "{}",
  })).status, 401, "privileged local-process control must reject an unauthenticated caller");
