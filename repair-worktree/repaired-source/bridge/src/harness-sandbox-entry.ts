...
function allowedRelayPath(url: string): boolean {
  let pathname: string;
...
function relay(socketPath: string, request: IncomingMessage, response: ServerResponse): void {
  if (!allowedRelayPath(request.url ?? "")) return fail(response, 404, "isolated relay path is not allowed");
...
async function consumeProxyToken(target: string, sandboxRoot: string): Promise<string> {
  const canonicalRoot = await realpath(sandboxRoot);
  const canonical = await realpath(target);
  const relative = path.relative(canonicalRoot, canonical);
  if (relative.startsWith("..") || path.isAbsolute(relative)) throw new Error("proxy token file resolves outside sandbox root");
  const info = await lstat(canonical);
  ...
  if (!/^[a-f0-9]{48}$/.test(token)) throw new Error("proxy token format is invalid");
