...
async function call(client: StdioMcpClient, name: string, args: Record<string, unknown>, timeoutMs = 120_000): Promise<Payload> {
  return parseToolPayload(await client.callTool(name, args, timeoutMs));
}

async function expectToolError(client: StdioMcpClient, name: string, args: Record<string, unknown>): Promise<Payload> {
...
async function waitTerminal(client: StdioMcpClient, taskId: string): Promise<Payload> {
...
try {
  ...
  try {
    ...
    const proLeaf = (proPlan.leaves as Array<Record<string, unknown>>)[0]!;
    assert.equal(proLeaf.complexity, "large");
    assert.equal(proLeaf.model, "deepseek-v4-pro");
    assert.equal((proLeaf.budget as Record<string, unknown>).enforcement, "hard");
    assert.equal((proLeaf.budget as Record<string, unknown>).gatePolicy, "input_output_tokens");
    assert.equal((proLeaf.budget as Record<string, unknown>).ceilingPolicy, "unbounded");
    ...
    assert.equal((splitAdvice.candidates as Array<Record<string, unknown>>).length, 1);
    ...
    process.stdout.write(`${JSON.stringify({
      ...
      proComplexBudgetEnforcement: (proLeaf.budget as Record<string, unknown>).enforcement,
      proComplexTokenGatePolicy: (proLeaf.budget as Record<string, unknown>).gatePolicy,
      adaptiveSplitTools: true,
      taskId: "mcp-task",
      commit: committed.commit,
    }, null, 2)}\n`);
  } finally {
    try { await call(client, "harness_monitor_stop", {}); } catch { /* fixture cleanup remains best effort */ }
    await client.close();
  }
} finally {
