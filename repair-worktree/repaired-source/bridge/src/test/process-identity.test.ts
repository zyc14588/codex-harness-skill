...
test("signals require exact PID lifetime, executable hash, and process-group leadership", { skip: process.platform !== "linux" }, async () => {
  ...
  try {
    ...
    await assert.rejects(
      signalVerifiedProcessGroup({ ...identity, processGroupId: identity.processGroupId + 1 }, "SIGTERM"),
      /non-leader process group/,
    );
    ...
    while (Date.now() < deadline && await processIdentityMatches(identity)) await sleep(20);
    assert.equal(await processIdentityMatches(identity), false);
  } finally {
    if (await processIdentityMatches(identity)) await signalVerifiedProcessGroup(identity, "SIGKILL");
  }
});
