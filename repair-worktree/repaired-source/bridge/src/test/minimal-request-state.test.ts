...
function taskRecord(root: string, id: string): TaskRecord {
...
async function withStateTask<T>(id: string, run: (root: string, task: TaskRecord) => Promise<T>): Promise<T> {
  ...
  try {
    ...
    if (previous === undefined) delete process.env.CODEX_HARNESS_CONFIG;
    else process.env.CODEX_HARNESS_CONFIG = previous;
    await rm(root, { recursive: true, force: true });
  }
}

function requestWithTools(names: string[]): Record<string, unknown> {
  return {
...
test("attributes a runner-present but wire-missing core tool to serialization", async () => {
  await withStateTask("serialization-wire-missing", async (_root, task) => {
    ...
    if (claim.ok) assert.fail("expected serialization mismatch");
...
test("rejects impossible force/tool-plane telemetry combinations", () => {
  ...
  } as TaskRecord), /legacy minimal_tool_plane with zero force count/u);
  assert.throws(() => assertMinimalRequestInvariant({
    minimalMutationForceCount: 1,
    minimalMutationForcedTools: ["mcp__bridge__capability_catalog"],
  } as TaskRecord), /without a forced core mutation tool/u);
});
