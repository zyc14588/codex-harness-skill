...
const RUNTIME_LLAMA_KEYS: ReadonlySet<keyof LlamaCppConfig> = new Set([
...
function normalizeRuntimeLlama(config: BridgeConfig, value: unknown, fallback: LlamaCppConfig, rejectImmutable = false): LlamaCppConfig {
  const raw = value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : {};
  if (rejectImmutable) {
    const forbidden = Object.keys(raw).filter((key) => !RUNTIME_LLAMA_KEYS.has(key as keyof LlamaCppConfig));
    if (forbidden.length) throw new Error(`runtime llama.cpp controls cannot change installation-owned fields: ${forbidden.sort().join(", ")}`);
  }
  const candidate: Record<string, unknown> = { ...config.llamaCpp };
  for (const key of RUNTIME_LLAMA_KEYS) {
    if (raw[key] !== undefined) candidate[key] = raw[key];
    else candidate[key] = fallback[key];
  }
