...
import { loadTask, taskDirectory, updateTask, withNamedLock } from "./store.js";
...
interface ProgressiveAuditEvent {
  schemaVersion: 1;
  at: string;
  taskId: string;
  planId: string;
  leafId: string;
  kind: "capability_enable" | "tool_call";
  capability: ProgressiveToolCapability;
  tool?: string;
  result: "enabled" | "already_enabled" | "denied" | "completed" | "failed";
  reason?: string;
}

const CAPABILITIES: Record<ProgressiveToolCapability, { description: string; tools: string[] }> = {
  repository_read: {
...
function optionalInteger(input: Record<string, unknown>, field: string, fallback: number): number {
  const value = input[field];
...
function capabilityValue(value: unknown): ProgressiveToolCapability {
...
function statePath(config: BridgeConfig, taskId: string): string {
  return path.join(taskDirectory(config, taskId), "progressive-tools.json");
}

function auditPath(config: BridgeConfig, taskId: string): string {
  return path.join(taskDirectory(config, taskId), "progressive-tools-audit.ndjson");
}

async function readState(config: BridgeConfig, taskId: string): Promise<ProgressiveState> {
  const target = statePath(config, taskId);
  if (!await pathExists(target)) {
    return { schemaVersion: 1, taskId, enabled: [], updatedAt: nowIso() };
  }
  const raw = JSON.parse(await readFile(target, "utf8")) as Partial<ProgressiveState>;
  const enabled = Array.isArray(raw.enabled)
    ? raw.enabled.filter((item): item is ProgressiveToolCapability => item === "repository_read" || item === "verification" || item === "git_inspect")
    : [];
  return { schemaVersion: 1, taskId, enabled: [...new Set(enabled)], updatedAt: typeof raw.updatedAt === "string" ? raw.updatedAt : nowIso() };
}

async function appendAudit(config: BridgeConfig, event: ProgressiveAuditEvent): Promise<void> {
  await ensureDir(taskDirectory(config, event.taskId));
  await appendFile(auditPath(config, event.taskId), `${JSON.stringify(event)}\n`, { encoding: "utf8", mode: 0o600 });
}

function assertUsableTask(task: TaskRecord): void {
  if (task.executor !== "harness" && task.effectiveExecutor !== "harness") throw new Error("progressive tools are available only to Harness leaves");
  if (task.harnessMode !== "minimal") throw new Error("progressive tools are available only in Harness minimal mode");
  if (task.status !== "queued" && task.status !== "running") throw new Error(`task is not active: ${task.status}`);
}

async function canonicalWorktree(task: TaskRecord): Promise<string> {
  return await realpath(task.worktreePath);
}

async function resolveReadablePath(task: TaskRecord, input: string): Promise<{ relative: string; absolute: string }> {
  ...
  if (!isWithin(candidate, worktree)) throw new Error(`path escapes worktree: ${input}`);
  const info = await lstat(candidate);
  if (info.isSymbolicLink()) throw new Error(`symbolic links are not readable through this tool: ${relative}`);
  if (!info.isFile()) throw new Error(`path is not a regular file: ${relative}`);
  const canonical = await realpath(candidate);
  if (!isWithin(canonical, worktree)) throw new Error(`resolved path escapes worktree: ${relative}`);
  return { relative, absolute: canonical };
}

async function commandResult(command: string, args: string[], task: TaskRecord, timeoutMs = 120_000, maxCaptureChars = 200_000): Promise<Record<string, unknown>> {
...
async function buildTools(config: BridgeConfig, task: TaskRecord, enabled: ProgressiveToolCapability[]): Promise<ToolDefinition[]> {
  ...
  if (enabled.includes("repository_read")) {
    base.push({
      ...
      invoke: async (input) => await auditToolCall(config, task, "repository_read", "repo_read_file", async () => {
        ...
        if (end < start || end - start > 1999) throw new Error("line range must be ordered and contain at most 2000 lines");
        const info = await lstat(target.absolute);
        if (info.size > 5_000_000) throw new Error("file exceeds the 5 MB progressive-read limit");
        const text = await readFile(target.absolute, "utf8");
        if (text.includes("\0")) throw new Error("binary/NUL file is not supported");
    ...
    base.push({
      ...
      invoke: async (input) => await auditToolCall(config, task, "repository_read", "repo_search", async () => {
        ...
        if (!Array.isArray(pathValues) || !pathValues.every((item) => typeof item === "string")) throw new Error("paths must be an array of strings");
        ...
        if (code !== 0 && code !== 1) throw new Error(`git grep failed: ${String(result.stderr)}`);
        return result;
      }),
    });
  }
  ...
  if (enabled.includes("git_inspect")) {
    ...
    base.push({
      ...
      invoke: async (input) => await auditToolCall(config, task, "git_inspect", "git_diff", async () => {
        ...
        if (input.statOnly === true) args.push("--stat");
        args.push("--");
...
function errorResponse(id: JsonRpcRequest["id"], code: number, message: string): Record<string, unknown> {
  ...
  const handle = async (request: JsonRpcRequest): Promise<void> => {
    ...
    try {
      if (request.method === "initialize") {
        ...
        send({
          ...
          result: {
            ...
            serverInfo: { name: "codex-harness-progressive-tools", version: "0.6.5" },
      ...
      if (request.method === "tools/call") {
        ...
        if (!tool) { send(errorResponse(id, -32602, `tool is not currently disclosed: ${name}`)); return; }
        try {
          const result = await tool.invoke(asObject(params.arguments));
          send({ jsonrpc: "2.0", id, result: jsonToolResult(result) });
          if (name === "capability_enable" && (result as { changed?: boolean }).changed === true) {
