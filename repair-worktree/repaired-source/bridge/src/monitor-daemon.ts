...
async function proxyRequest(config: BridgeConfig, request: IncomingMessage, response: ServerResponse, token: string, suffix: string, query: string): Promise<void> {
  ...
  if (!authorizeBearer(request.headers.authorization, token)) return json(response, 403, { error: "missing or invalid one-task proxy bearer" });
...
function operatorToken(){let token=sessionStorage.getItem("codexHarnessOperatorToken");if(!token){token=window.prompt("请输入监控操作员令牌")||"";if(token)sessionStorage.setItem("codexHarnessOperatorToken",token)}if(!token)throw new Error("缺少操作员令牌");return token}
async function api(path,options={}){const headers=new Headers(options.headers||{});headers.set("authorization","Bearer "+operatorToken());const r=await fetch(BASE+path,{...options,headers});const text=await r.text();let v;try{v=JSON.parse(text)}catch{v={error:text||"invalid JSON"}}if(!r.ok){if(r.status===401||r.status===403)sessionStorage.removeItem("codexHarnessOperatorToken");throw new Error(v.error?.message||v.error||("HTTP "+r.status))}return v}
...
async function refresh(){try{render(await api("/api/snapshot"))}catch(e){q("#connection").textContent="连接失败："+e.message}}refresh();setInterval(refresh,2000);
</script></body></html>`;
}
...
async function authorizedInternalTask(request: IncomingMessage, body: Record<string, unknown>): Promise<TaskRecord> {
  ...
  let task: TaskRecord;
  try { task = await loadTask(config, body.taskId); }
  catch { throw new HttpRequestError(403, "invalid or inactive task credential"); }
...
async function internalRequestState(request: IncomingMessage, response: ServerResponse, operation: string): Promise<void> {
...
const internalServer = http.createServer(async (request, response) => {
  try {
    ...
    if (state?.[1]) return await internalRequestState(request, response, state[1]);
    return json(response, 404, { error: "not found" });
  } catch (error) {
    const status = error instanceof HttpRequestError ? error.status : 500;
    if (!response.headersSent) json(response, status, { error: error instanceof Error ? error.message : String(error) });
