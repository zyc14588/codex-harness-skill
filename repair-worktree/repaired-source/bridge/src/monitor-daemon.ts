...
<section id="authPanel" class="auth-panel" aria-labelledby="authTitle"><h2 id="authTitle">操作员认证</h2><div class="small">请输入操作员密码（Bearer 令牌）后加载任务与预算。首次登录使用安装器生成的令牌；认证成功后可在“设置”中更换为自己的密码。</div><div class="auth-grid"><div><label for="operatorCredential">操作员密码 / 令牌</label><input id="operatorCredential" type="password" autocomplete="current-password" spellcheck="false" placeholder="至少 6 个字符"></div><button id="loginOperator" type="button">登录并加载控制台</button></div><div id="authMessage" class="message small"></div></section>
...
q("#saveOperatorCredential").onclick=async()=>{const next=q("#newOperatorCredential").value,confirmation=q("#confirmOperatorCredential").value,characters=Array.from(next).length,bytes=new TextEncoder().encode(next).length;if(next!==confirmation){setMessage("#credentialMessage","两次输入的新密码不一致",false);return}if(characters<6||bytes>16384||/\\s/u.test(next)){setMessage("#credentialMessage","新密码必须至少 6 个字符、至多 16384 个 UTF-8 字节且不含空白字符",false);return}try{await mutate("/api/operator-credential","POST",{newToken:next});sessionStorage.setItem("codexHarnessOperatorToken",next);q("#newOperatorCredential").value="";q("#confirmOperatorCredential").value="";setMessage("#credentialMessage","新操作员密码已保存；旧密码已立即失效")}catch(e){setMessage("#credentialMessage",String(e.message||e),false)}};
