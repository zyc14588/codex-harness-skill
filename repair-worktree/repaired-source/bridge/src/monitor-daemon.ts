...
function authenticationRequired(message="需要操作员令牌"){q("#connection").textContent=message;q("#authenticate").hidden=false}
function requestOperatorToken(){const token=(window.prompt("请输入监控操作员令牌")||"").trim();if(!token){authenticationRequired();return false}sessionStorage.setItem("codexHarnessOperatorToken",token);q("#authenticate").hidden=true;return true}
...
function render(s){latest=s;q("#connection").textContent="已连接 · v"+s.serviceVersion+" · "+new Date(s.generatedAt).toLocaleString("zh-CN");q("#authenticate").hidden=true;renderTasks(s);renderCosts(s);if(!q("#llamaModel").matches(":focus"))fillLlama(s.llamaCpp)}
...
q("#authenticate").onclick=()=>{if(requestOperatorToken())void refresh()};if(!storedOperatorToken())requestOperatorToken();void refresh();setInterval(()=>void refresh(),2000);
</script></body></html>`;
}
