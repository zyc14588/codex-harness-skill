...
> F-001–F-015 的原始封印证据保留为历史；runtime hotfix R4 已重跑本地、浏览器、安全、包装、来源与最终归档门禁。Provider 请求路径未修改，R2 的有界真实结果仅作继承回归证据；权威结果以 `release-status.json` 为准。
...
| F-007 | 严格 Flash/Pro/replay Mock、多轮 direct E2E、provider fail-fast、Dashboard 认证/密码轮换 | 90 项 component + direct + browser |
...
本地修复矩阵没有用 fixture 冒充真实 Provider。R4 没有修改 Provider 请求路径，也没有重新产生真实 API 调用、Token 或费用；`evidence/09` 明确保留为 R2 继承回归证据。R4 的 90 项测试、6 字符 operator 边界、Provider key 24 字节下限、真实浏览器、普通源码/provenance、两次确定性 ZIP 字节比较及解压复验均为 PASS。
