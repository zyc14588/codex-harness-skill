# 0.6.5 runtime hotfix R3 测试报告

报告状态：`DELIVERABLE_PASS / stable`。R3 使用 evidence/08 绑定当前 Dashboard、密码轮换、本地安全、浏览器和包级资格；evidence/09 是 Provider 路径未修改时继承的 R2 有界真实回归证据，不表述为 R3 重跑。
...
`evidence/08_RUNTIME_HOTFIX_CANDIDATE_LOCAL_VALIDATION.json` 绑定 89 项回归、direct/security/skill/package 和桌面/移动浏览器验证。`evidence/09_RUNTIME_HOTFIX_REAL_DEEPSEEK_REDACTED.json` 绑定 R2 的真实 Flash/Pro，且不包含 credential、prompt 或 reasoning 正文；R3 Provider 路径未变，本轮 API calls/tokens/cost 均为 0。
