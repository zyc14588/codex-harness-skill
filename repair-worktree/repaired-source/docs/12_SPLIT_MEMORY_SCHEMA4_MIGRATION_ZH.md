...
当前 hotfix R2 的 split-memory 窄测试、87 项全量回归、direct acceptance、动态 Harness、失败注入与真实 Provider 已独立通过；此前 stable 的结果只保留为历史证据。release bindings 与最终解压归档仍需完成后才能转为 stable。
