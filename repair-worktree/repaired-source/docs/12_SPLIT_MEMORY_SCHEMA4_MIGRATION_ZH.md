...
当前 hotfix R4 的 split-memory 窄测试、90 项全量回归、direct acceptance 与 package acceptance 已通过；R4 没有修改 split-memory 或 Provider/Harness 请求路径，R2 的动态、失败注入与真实 Provider 结果作为继承回归证据明确保留。release bindings、确定性双构建与最终解压归档也已完成。
