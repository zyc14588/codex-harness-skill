...
从 R6.0/R6.1/R6.2/R6.3 升级到 R6.4 时，配置 Schema 仍为 v6；安装器会替换 Bridge 管理的 minimal preset/profile。拆分记忆继续使用内部 schema v3：schema-v1/v2 画像不再生效并会在后续写入时归档；R6.2/R6.3 schema-v3 记录保留，但基础设施故障不参与任务形状学习。无需人工删除证据。
...
```bash
~/.local/share/codex-harness-bridge/0.6.4/scripts/monitor.sh status
~/.local/share/codex-harness-bridge/0.6.4/scripts/monitor.sh snapshot
~/.local/share/codex-harness-bridge/0.6.4/scripts/monitor.sh stop
```
...
R6.4 会自动隔离 R6.0 schema-v1 与 R6.1 schema-v2 旧画像。R6.2/R6.3 schema-v3 证据会继续存在，但基础设施失败不影响建议。只有在确实需要人为清空全部任务族经验时，才应先停止活动任务，再备份并删除：
...
Bridge 管理的 R6.4 minimal headless profile应已禁用 `session-title-llm`。若外部组合仍产生标题辅助调用，任务可显示 `minimalMutationAuxiliaryBypassCount > 0` 和 `session_title_auxiliary`；该请求不得触发 `minimal_tool_plane`，随后真正的施工请求仍必须看到 `minimalMutationForceCount > 0`。
...
```bash
~/.local/share/codex-harness-bridge/0.6.4/scripts/doctor.sh
~/.local/share/codex-harness-bridge/0.6.4/scripts/acceptance.sh
~/.local/share/codex-harness-bridge/0.6.4/scripts/uninstall.sh
```
