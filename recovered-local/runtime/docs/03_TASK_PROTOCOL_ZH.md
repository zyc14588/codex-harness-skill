...
`tool_protocol`、`minimal_tool_plane`、`provider_transport` 与 `no_effect` 表示 Provider/Agent/上游传输链异常，而不是叶子任务过大。它们会：
...
R6.0 schema-v1 与 R6.1 schema-v2 画像不会参与 R6.4 advice，并在后续写入时归档到 `legacy/`。R6.2/R6.3 的 schema-v3 记录可以保留为证据；其中标记为基础设施故障的事件不改变规模、复杂度或 Token 建议。
...
reviewed/current/verified fingerprint 必须一致。不同并行叶子必须分别审查、验证和提交。
