...
Usage: $0 [options]
  ...
  Custom llama-cli/llama-server binaries and arguments are editable in the Web UI
  ...
  --skip-self-tests         Skip deterministic package tests; actual doctor still runs
...
while (($#)); do
  case "$1" in
    --harness-root|--allowed-root|--install-root|--config|--monitor-port|--dsh-home|--llama-base-url|--llama-model)
      need_value "$@"
      case "$1" in
...
for bin in node git codex python3 tar sha256sum timeout; do
...
for artifact in \
  ...
  scripts/hash-tree.mjs scripts/render-minimal-harness.py harness/minimal/profile/cordis.patch.yml harness/minimal/preset/agent.cordis.yml.in; do
...
(
  cd "$SOURCE_ROOT"
  tar --exclude='./bridge/node_modules' --exclude='./bridge/package-lock.json' --exclude='./.git' -cf - .
...
python3 - "$CONFIG_PATH" "$STATE_ROOT_DEFAULT" "$HARNESS_ROOT" "$HARNESS_CLI" "$HARNESS_BUILD_ROOT" "$HARNESS_COMMIT" "$HARNESS_BUILD_SHA256" "$ALLOW_DIRTY_HARNESS" "$DSH_HOME_TARGET" "$MONITOR_PORT" "$LLAMA_MODE" "$LLAMA_BASE_URL" "$LLAMA_MODEL" "${ALLOWED_ROOTS[@]}" <<'PY'
import copy, json, os, sys
(config_path, state_default, harness_root, harness_cli, build_root, commit, build_sha, allow_dirty, dsh_home,
 monitor_port, llama_mode, llama_url, llama_model, *roots) = sys.argv[1:]
...
default_env=[
    "PATH","HOME","USER","LOGNAME","SHELL","LANG","LC_ALL","TERM","COLORTERM",
    "XDG_CONFIG_HOME","XDG_DATA_HOME","XDG_STATE_HOME","DEEPSEEK_API_KEY","DEEPSEEK_BASE_URL",
    "DEEPSEEK_SEARCH_BASE_URL","DSH_MODEL","LLAMA_CPP_API_KEY","HTTP_PROXY","HTTPS_PROXY","ALL_PROXY",
    "NO_PROXY","NODE_USE_ENV_PROXY","NODE_EXTRA_CA_CERTS","SSL_CERT_FILE"
]
if not isinstance(config.get("passEnvironment"), list): config["passEnvironment"]=default_env.copy()
for name in default_env:
    if name not in config["passEnvironment"]: config["passEnvironment"].append(name)
...
for k,v in {
    ...
    "cliBinary":"llama-cli",
    "cliArgs":["-p","{{PROMPT}}","-n","{{MAX_TOKENS}}","--temp","0"],
...
echo "[4/14] Installing Bridge-managed Harness minimal profile and progressive-tool preset"
...
if ((SKIP_SELF_TESTS)); then
  ...
  echo "[5/14] Running 60 prebuilt unit tests"
