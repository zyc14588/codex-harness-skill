...
import { atomicWriteJson, ensureDir, pathExists, processAlive, readJson, sleep } from "./util.js";
...
async function reclaimableLock(lockPath: string): Promise<boolean> {
  ...
  try {
    ...
    if (Number.isInteger(owner.pid) && Number(owner.pid) > 0) {
      if (processAlive(Number(owner.pid))) return false;
      return ageMs >= 1_000;
    }
...
export async function withNamedLock<T>(config: BridgeConfig, lockName: string, timeoutMs: number, fn: () => Promise<T>): Promise<T> {
  ...
  while (true) {
    try {
      ...
      try {
        const owner: LockOwner = { pid: process.pid, acquiredAt: new Date().toISOString(), lockName };
