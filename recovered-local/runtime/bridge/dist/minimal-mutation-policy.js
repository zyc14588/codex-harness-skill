 ...
 */
export function isSessionTitleAuxiliaryRequest(requestBody) {
    if (allowedToolNamesFromRequest(requestBody).size !== 0)
        return false;
    const maxTokens = requestBody.max_tokens;
    if (typeof maxTokens === "number" && Number.isFinite(maxTokens) && maxTokens > 128)
        return false;
