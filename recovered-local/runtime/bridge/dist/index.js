...
const booleanSchema = (defaultValue) => defaultValue === undefined ? { type: "boolean" } : { type: "boolean", default: defaultValue };
...
function stringArray(input, field, fallback) {
    const value = input[field];
...
const tools = [
    ...
    },
    {
        ...
        invoke: async (input) => await createControllerPlan(input),
    },
    {
        ...
        invoke: async (input) => await controllerSplitAdvice(requiredString(input, "repoRoot"), input.candidates),
    },
    {
    ...
    },
    {
        ...
        invoke: async (input) => await reviewTask(requiredString(input, "taskId"), requiredString(input, "decision"), stringArray(input, "reviewedPaths"), typeof input.notes === "string" ? input.notes : ""),
    },
    {
        name: "harness_repair",
    ...
    },
    {
        ...
        invoke: async (input) => await verifyTask(requiredString(input, "taskId"), input.commands === undefined ? undefined : stringArray(input, "commands"), optionalInteger(input, "timeoutSeconds") ?? 1800),
    },
    {
        name: "harness_commit",
...
async function handle(request) {
    ...
    try {
        if (request.method === "initialize") {
            ...
            send({
                ...
                result: {
                    ...
                    serverInfo: { name: "codex-harness-bridge", version: "0.6.4" },
        ...
        if (request.method === "tools/call") {
            ...
            if (!tool) {
                send(errorResponse(id, -32602, `unknown tool: ${name}`));
                return;
            }
            try {
