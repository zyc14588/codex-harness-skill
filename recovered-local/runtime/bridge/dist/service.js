...
import { budgetWithin, DEEPSEEK_PRO_MODEL, defaultConfigPath, loadConfig, normalizeTaskBudget, resolveHarnessLauncher, sanitizedEnvironment } from "./config.js";
import { assertTaskWorktreeIdentity, binaryPatch, changedPaths, commitLog, createCommit, createWorktree, deleteBranch, diffStat, findLeaseSymlinkIntersections, findOutOfScope, gitlinkPathsAtCommit, gitTopLevel, readRepoFile, removeWorktree, resolveCommit, stagedPaths, symlinkPathsAtCommit, textFileAtCommit, unsafeChangedGitlinkPaths, unsafeChangedSymlinkPaths, workingTreePaths, } from "./git.js";
...
import { assertDisjointLeases, boundedStringList, boundedText, ensureDir, isWithin, jsonToolResult, normalizeRepoRelative, nowIso, pathExists, processAlive, runProcess, safeTaskId, sha256PathTree, tailText, validateLeasePattern, } from "./util.js";
...
function unsafeHarnessEnvironmentNames(text) {
    ...
    for (const rawLine of text.split(/\r?\n/)) {
        ...
        if (!match?.[1])
            continue;
        const name = match[1];
        if (name.startsWith("DSH_") || ["DEEPSEEK_API_KEY", "DEEPSEEK_BASE_URL", "DEEPSEEK_SEARCH_BASE_URL"].includes(name)) {
            unsafe.add(name);
        }
    }
    return [...unsafe].sort();
...
function validatedHarnessUpstreamBaseUrl(config) {
    const raw = (process.env.DEEPSEEK_BASE_URL ?? "https://api.deepseek.com").trim().replace(/\/+$/, "");
    let value;
    try {
        value = new URL(raw);
    }
    catch {
        throw new Error("DEEPSEEK_BASE_URL must be a valid absolute URL");
    }
    if (value.protocol !== "http:" && value.protocol !== "https:")
        throw new Error("DEEPSEEK_BASE_URL must use http or https");
    if (value.username || value.password)
        throw new Error("DEEPSEEK_BASE_URL must not embed credentials; use DEEPSEEK_API_KEY");
...
function terminateRecordedProcess(pid) {
...
async function normalizedLeaf(config, repoRoot, defaults, maximum, proComplexDefaults, llama, input) {
    ...
    const proComplexLeaf = executor === "harness" && complexity === "large" && requestedModel === DEEPSEEK_PRO_MODEL;
    ...
    const taskFamily = boundedText(input.taskFamily?.trim() || defaultTaskFamily(mode, harnessWritePaths, executor, requestedModel), `leaf ${id} taskFamily`, 500);
...
export async function doctor(probeHarness) {
    ...
    try {
        ...
        if (probeHarness) {
            ...
            const minimalStructureOk = minimalIntegration.profileExists && minimalIntegration.profileManaged && minimalIntegration.presetExists && minimalIntegration.presetManaged && minimalIntegration.progressiveToolServerExists;
            probeOk = version.code === 0 && standardComposition.code === 0 && minimalComposition.code === 0 && minimalStructureOk;
            checks.harnessProbe = {
                ok: probeOk,
...
export async function startTask(input) {
    ...
    return await withMutationLock(config, async () => {
        ...
        const projectEnv = await textFileAtCommit(repoRoot, plan.baseCommit, ".env");
        const unsafeEnvNames = projectEnv ? unsafeHarnessEnvironmentNames(projectEnv) : [];
        if (unsafeEnvNames.length)
            throw new Error(`tracked .env controls Harness bootstrap variables and is forbidden: ${unsafeEnvNames.join(", ")}`);
        if (config.monitor.enabled)
            await ensureMonitorRunning(config, defaultConfigPath());
...
export async function taskStatus(taskId) {
    ...
    const workerAlive = processAlive(task.workerPid);
    ...
    if (livenessDecision === "observe-dead" || livenessDecision === "clear-dead-observation" || livenessDecision === "orphan") {
        task = await updateTask(config, id, (current) => {
            ...
            const currentAlive = processAlive(current.workerPid);
    ...
    return {
        ...
        status: task.status,
        phase: task.phase,
        workerAlive: processAlive(task.workerPid),
        harnessAlive: processAlive(task.harnessPid),
        createdAt: task.createdAt,
        startedAt: task.startedAt,
        completedAt: task.completedAt,
        changedPaths: task.changedPaths,
...
export async function verifyTask(taskId, commands, timeoutSeconds = 1800) {
    ...
    return await withWorktreeLock(config, initial.worktreePath, async () => {
        ...
        if (!fingerprintStable)
            task.error = "authoritative verification modified the reviewed diff; restore or re-review before adoption";
        await saveTask(config, task);
        await updatePlan(config, task.planId, (plan) => {
            const leaf = plan.leaves.find((candidate) => candidate.id === task.leafId);
            if (!leaf)
...
export async function commitTask(taskId, message) {
    ...
    return await withWorktreeLock(config, initial.worktreePath, async () => {
        ...
        return {
            ...
            reviewedFingerprint: task.reviewedFingerprint, verifiedFingerprint: task.verifiedFingerprint,
...
export async function cancelTask(taskId) {
    ...
    terminateRecordedProcess(task.harnessPid);
    terminateRecordedProcess(task.workerPid);
    await updatePlan(config, task.planId, (plan) => {
        const leaf = plan.leaves.find((candidate) => candidate.id === task.leafId);
        if (leaf) {
            leaf.status = "rejected";
...
export async function cleanupTask(taskId, force, deleteTaskBranch) {
    ...
    return await withMutationLock(config, async () => {
        ...
        return await withWorktreeLock(config, initial.worktreePath, async () => {
            ...
            if (task.status === "cancelled" && (processAlive(task.workerPid) || processAlive(task.harnessPid))) {
...
export async function monitorSnapshot(limit) {
    ...
    if (config.monitor.enabled) {
        await ensureMonitorRunning(config, defaultConfigPath());
        try {
            const response = await fetch(`${monitorBaseUrl(config)}/api/snapshot?limit=${bounded}`, { signal: AbortSignal.timeout(2_000) });
            if (response.ok)
