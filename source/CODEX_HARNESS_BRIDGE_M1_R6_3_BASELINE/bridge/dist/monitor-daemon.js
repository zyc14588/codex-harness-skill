...
import { appendUsageEvent, budgetExceededReason, calculateCostCny, calculateCostUsd, clearBudgetMarker, estimateTokens, markBudgetExceeded, parseProviderUsage, pricingForModel, projectedBudgetExceededReason, usageEventId, usageForBudgetGroup, writeUsageSnapshot, } from "./telemetry.js";
...
function textFromMessages(value) {
    if (!Array.isArray(value))
...
function projectedOutput(body, budget, config) {
...
function textualContent(value) {
    ...
    if (Array.isArray(record.content))
        return record.content.map(textualContent).join("");
    return "";
}
        ...
        if (preliminaryPolicy.applied || preliminaryPolicy.reason) {
            ...
            if (mutationPolicy.applied) {
                ...
                await updateTask(config, latest.id, (current) => {
                    ...
                    if (current.infrastructureFailureKind === "minimal_tool_plane")
...
function mutationAuthorized(request, baseUrl) {
...
const server = http.createServer(async (request, response) => {
    try {
        ...
        if (url.pathname === "/health")
            return json(response, 200, { ok: true, service: "codex-harness-monitor", pid: process.pid, configPath: defaultConfigPath(), version: VERSION });
        ...
        if (blocked?.[1]) {
            ...
            if (!task)
                return json(response, 403, { error: "invalid task search token" });
