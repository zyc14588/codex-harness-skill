...
test("runtime budget policy and group override are bounded, auditable, and immediately effective", async () => {
  ...
  try {
    ...
    const policy = await setBudgetPolicy(config,
      { maxApiCalls: 6, maxInputTokens: 50_000, maxOutputTokens: 5_000, maxCostCny: 1.2 },
      { maxApiCalls: 20, maxInputTokens: 500_000, maxOutputTokens: 50_000, maxCostCny: 10 },
      { maxApiCalls: 150, maxInputTokens: 5_000_000, maxOutputTokens: 600_000, maxCostCny: 500 },
      "unit test policy", "dashboard");
    ...
    const referenceOnly = await setBudgetPolicy(config, policy.budgetPolicy.defaultHarnessBudget,
      { ...policy.budgetPolicy.maximumHarnessBudget, maxApiCalls: 1_000_000, maxCostCny: 100_000 }, policy.budgetPolicy.defaultProComplexBudget,
      "reference thresholds may exceed installation references", "dashboard");
    assert.equal(referenceOnly.budgetPolicy.maximumHarnessBudget.maxCostCny, 100_000);
    await assert.rejects(() => setBudgetPolicy(config, policy.budgetPolicy.defaultHarnessBudget,
      { ...policy.budgetPolicy.maximumHarnessBudget, maxInputTokens: config.controller.maximumHarnessBudget.maxInputTokens + 1 }, policy.budgetPolicy.defaultProComplexBudget,
      "token gate too high", "dashboard"), /operator ceiling/);
    ...
    const override = await setBudgetOverride(config, "group-a",
      { maxApiCalls: 8, maxInputTokens: 60_000, maxOutputTokens: 6_000, maxCostCny: 2 },
      "active task top-up", "dashboard");
...
test("runtime llama.cpp control supports CLI and managed server custom binaries but pins fallback model", async () => {
  ...
  try {
    ...
    assert.equal(cli.cliBinary, "/opt/custom/llama-cli-special");
    assert.deepEqual(cli.cliArgs.slice(-2), ["--json", "{{OUTPUT_JSON_FILE}}"]);
    assert.equal((await effectiveLlamaConfig(config)).enabled, true);
    await assert.rejects(() => setLlamaRuntimeConfig(config, { ...cli, fallbackModel: "other-model" }, "dashboard"), /pinned/);
  } finally { await rm(root, { recursive: true, force: true }); }
});
