...
import type { BridgeConfig, PricingEntry, TaskBudget, TaskRecord, UsageTotals } from "./types.js";
...
const MAX_PROVIDER_OUTPUT_TOKENS = 10_000_000;
const MAX_CAPTURE_BYTES = 24_000_000;
...
function taskByProxyToken(tasks: TaskRecord[], token: string): TaskRecord | undefined {
  return tasks.find((task) => task.proxyToken === token && (task.status === "queued" || task.status === "running"));
}

function upstreamUrl(base: string, suffix: string, query: string): URL {
  ...
  return value.map((item) => {
    ...
    if (Array.isArray(content)) return content.map((part) => {
      ...
      if (part && typeof part === "object") {
        ...
        return typeof entry.text === "string" ? entry.text : JSON.stringify(entry);
      }
      return String(part ?? "");
    }).join("\n");
...
function projectedOutput(body: Record<string, unknown>, budget: TaskBudget, config: BridgeConfig): number {
  const raw = body.max_tokens ?? body.max_completion_tokens;
  const requested = typeof raw === "number" && Number.isFinite(raw) ? Math.max(0, Math.floor(raw)) : budget.maxOutputTokens;
  void config;
  return Math.min(requested, budget.maxOutputTokens, MAX_PROVIDER_OUTPUT_TOKENS);
}
...
function clampRequestBody(
  ...
  const allowed = Math.max(0, Math.min(projectedOutput(body, budget, config), allowedByTokens, allowedByCost ?? Number.MAX_SAFE_INTEGER));
  if ("max_completion_tokens" in body) body.max_completion_tokens = allowed;
...
function textualContent(value: unknown): string {
  ...
  if (Array.isArray(record.content)) return record.content.map(textualContent).join("");
  return "";
}
...
async function proxyRequest(config: BridgeConfig, request: IncomingMessage, response: ServerResponse, token: string, suffix: string, query: string): Promise<void> {
  const task = taskByProxyToken(await listTasks(config), token);
  if (!task) return json(response, 403, { error: "invalid or inactive task proxy token" });
  ...
  await withNamedLock(config, `budget:${task.budgetGroupId}`, Math.max(30_000, task.runtimeSeconds * 1_000), async () => {
    const latest = await loadTask(config, task.id);
    ...
    const preliminaryPolicy = applyMinimalMutationPolicy(latest, parsed, [], model);
    if (preliminaryPolicy.applied || preliminaryPolicy.reason) {
      ...
      const mutationPolicy = applyMinimalMutationPolicy(latest, parsed, currentChangedPaths, model);
      if (mutationPolicy.reason) {
        ...
        await broadcast(config);
        return json(response, 502, { error: { message: policyFailure, type: "minimal_mutation_preflight" } });
      }
      if (mutationPolicy.applied) {
        ...
        await updateTask(config, latest.id, (current) => {
          current.minimalMutationForceCount = (current.minimalMutationForceCount ?? 0) + 1;
          current.minimalMutationForcedTools = [...new Set([...(current.minimalMutationForcedTools ?? []), ...mutationPolicy.toolNames])];
          current.minimalMutationPolicyVersion = MINIMAL_MUTATION_POLICY_VERSION;
          current.minimalMutationLastAt = nowIso();
          if (current.infrastructureFailureKind === "minimal_tool_plane") delete current.infrastructureFailureKind;
    ...
    const inputEstimate = estimateTokens(textFromMessages(providerRequestBody.messages ?? providerRequestBody.input ?? providerRequestBody.prompt ?? providerRequestBody), config.monitor.charsPerEstimatedToken);
    ...
    const clamped = clampRequestBody(config, providerRequestBody, totals, budget, model, inputEstimate);
    ...
    try {
      const upstreamHeaders = safeRequestHeaders(request);
      ...
      if (!["GET", "HEAD"].includes(request.method ?? "")) init.body = clamped.body;
      ...
      if (upstream.body) {
        ...
        while (true) {
          ...
          if (now - lastOutputEstimateAt >= LIVE_BROADCAST_INTERVAL_MS) {
            ...
            const liveOutput = estimateTokens(responseTextFromCapture(contentType, currentCapture), config.monitor.charsPerEstimatedToken);
    ...
    const estimatedOutput = estimateTokens(responseTextFromCapture(contentType, capture), config.monitor.charsPerEstimatedToken);
    ...
    await appendUsageEvent(latest, {
      ...
      httpStatus: streamError ? 502 : upstream.status, latencyMs: Date.now() - started,
      ...(provider ? {
    ...
    if (streamError) {
      return json(response, 502, { error: { message: streamError, type: "tool_protocol_error" } });
    }
...
<div class="grid2"><div class="panel"><h2>全局预算策略</h2><div class="section-note small">输入 Token 与输出 Token 是唯一执行门禁。API 调用次数和人民币费用只用于参考、告警与人工治理，不会单独中止叶子任务。</div><div class="grid2"><div><h3>普通叶子默认 Token 门禁</h3><div id="defaultBudgetFields"></div></div><div><h3>普通叶子运行时 Token 上限</h3><div id="maximumBudgetFields"></div></div><div class="wide advisory-box"><h3>Pro 复杂叶子默认高 Token 门禁</h3><div class="small muted">复杂叶子不受普通 operator ceiling 限制，但仍以自身冻结的输入/输出 Token 数量作为硬门禁；API 调用与费用字段仅供参考。</div><div id="proBudgetFields" class="grid3"></div></div></div><label>变更原因</label><input id="policyReason" maxlength="2000" placeholder="例如：调整复杂叶子的输入/输出 Token 门禁"><div class="actions"><button id="savePolicy">保存全局预算</button></div><div id="policyMessage" class="message small"></div></div>
<div class="panel"><h2>任务预算覆盖</h2><label>Budget group</label><select id="budgetGroup"></select><div id="overrideBudgetFields"></div><label>变更原因</label><input id="overrideReason" maxlength="2000" placeholder="例如：活动叶子补充预算"><div class="actions"><button id="saveOverride">应用/更新覆盖</button><button class="secondary" id="clearOverride">清除覆盖</button></div><div id="overrideMessage" class="message small"></div></div></div>
...
<div class="grid2"><div><label>模式</label><select id="llamaModeInput"><option value="external_server">外部 llama-server</option><option value="managed_server">Bridge 托管 llama-server</option><option value="cli">llama-cli</option></select></div><div><label>模型标识</label><input id="llamaModel"></div><div><label>工作目录（可留空）</label><input id="llamaWorkingDirectory"></div><div><label>请求超时（秒）</label><input id="llamaTimeout" type="number" min="1"></div></div>
<h3>Server 模式</h3><div class="grid2"><div><label>OpenAI-compatible Base URL</label><input id="llamaBaseUrl"></div><div><label>API Key 环境变量名</label><input id="llamaApiKeyEnv"></div><div><label>llama-server 可执行文件</label><input id="llamaServerBinary"></div><div><label class="switch"><input id="llamaServerAutoStart" type="checkbox">任务需要时自动启动托管服务</label></div><div class="wide"><label>llama-server 参数（JSON 字符串数组）</label><textarea id="llamaServerArgs"></textarea></div><div><label>启动健康检查超时（秒）</label><input id="llamaServerStartupTimeout" type="number" min="1"></div></div>
<h3>CLI 模式</h3><div class="grid2"><div><label>llama-cli 可执行文件</label><input id="llamaCliBinary"></div><div class="wide"><label>llama-cli 参数（JSON 字符串数组）</label><textarea id="llamaCliArgs"></textarea><div class="small muted">支持 {{PROMPT}}、{{PROMPT_FILE}}、{{OUTPUT_JSON_FILE}}、{{MAX_TOKENS}}、{{MODEL}}、{{WORKTREE}}</div></div></div>
...
async function api(path,options={}){const r=await fetch(BASE+path,options);const text=await r.text();let v;try{v=JSON.parse(text)}catch{v={error:text||"invalid JSON"}}if(!r.ok)throw new Error(v.error?.message||v.error||("HTTP "+r.status));return v}
...
function llamaForm(){let serverArgs,cliArgs;try{serverArgs=JSON.parse(q("#llamaServerArgs").value);cliArgs=JSON.parse(q("#llamaCliArgs").value)}catch{throw new Error("启动参数必须是合法 JSON 字符串数组")}return {enabled:q("#llamaEnabledInput").checked,autoRouteSimpleLeaves:q("#llamaAutoRoute").checked,mode:q("#llamaModeInput").value,baseUrl:q("#llamaBaseUrl").value,apiKeyEnv:q("#llamaApiKeyEnv").value,model:q("#llamaModel").value,workingDirectory:q("#llamaWorkingDirectory").value,serverBinary:q("#llamaServerBinary").value,serverArgs,serverAutoStart:q("#llamaServerAutoStart").checked,serverStartupTimeoutSeconds:Number(q("#llamaServerStartupTimeout").value),cliBinary:q("#llamaCliBinary").value,cliArgs,requestTimeoutSeconds:Number(q("#llamaTimeout").value),maxFilesPerTask:Number(q("#llamaMaxFiles").value),maxContextFiles:Number(q("#llamaMaxContextFiles").value),maxContextBytes:Number(q("#llamaMaxContextBytes").value),maxFileBytes:Number(q("#llamaMaxFileBytes").value),maxOutputTokens:Number(q("#llamaMaxOutputTokens").value),fallbackEnabled:q("#llamaFallbackEnabled").checked,fallbackModel:"deepseek-v4-flash"}}
...
api("/api/snapshot").then(render).catch(e=>q("#connection").textContent="连接失败："+e.message);const es=new EventSource(BASE+"/events");es.addEventListener("snapshot",e=>render(JSON.parse(e.data)));es.onerror=()=>q("#connection").textContent="实时连接中断，正在重连…";
</script></body></html>`;
}

function mutationAuthorized(request: IncomingMessage, baseUrl: string): string | undefined {
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === "/health") return json(response, 200, { ok: true, service: "codex-harness-monitor", pid: process.pid, configPath: defaultConfigPath(), version: VERSION });
    ...
    if ((request.method === "POST" || request.method === "DELETE") && url.pathname.startsWith("/api/")) {
      const denied = mutationAuthorized(request, baseUrl);
    ...
    if (url.pathname === "/api/llama/config" && request.method === "POST") {
      const body = await readControlBody(request);
      const settings = await setLlamaRuntimeConfig(config, body.config, "dashboard");
      await broadcast(config);
    ...
    if (blocked?.[1]) {
      ...
      if (!task) return json(response, 403, { error: "invalid task search token" });
    ...
    if (proxy?.[1]) return await proxyRequest(config, request, response, proxy[1], proxy[2] ?? "", url.search);
    if (url.pathname === "/") {
