...
import type { BridgeConfig, TaskBudget, TaskRecord, UsageTotals } from "./types.js";
...
interface MonitorPidRecord {
  ...
  configPath: string;
  baseUrl: string;
  /** Linux /proc start-time tick, used to reject a reused PID. */
...
async function pidRecordMatchesProcess(record: MonitorPidRecord): Promise<boolean> {
  ...
  if (!record.processStartTimeTicks && !record.daemonPath) return false;
  if (record.processStartTimeTicks && identity.startTimeTicks !== record.processStartTimeTicks) return false;
  if (record.daemonPath) {
    const expected = path.resolve(record.daemonPath);
    if (!identity.commandLine?.some((value) => path.resolve(value) === expected)) return false;
  }
  return true;
}
...
export async function ensureMonitorRunning(config: BridgeConfig, configPath: string): Promise<{ ok: boolean; pid?: number; baseUrl: string; started: boolean }> {
  ...
  return await withNamedLock(config, "monitor-start-stop", 30_000, async () => {
    ...
    if (existing.ok) return { ok: true, ...(existing.pid === undefined ? {} : { pid: existing.pid }), baseUrl: existing.baseUrl, started: false };
...
export async function stopMonitor(config: BridgeConfig): Promise<{ ok: boolean; stopped: boolean; pid?: number; baseUrl: string }> {
  return await withNamedLock(config, "monitor-start-stop", 30_000, async () => {
    ...
    while (Date.now() < gracefulDeadline && processAlive(pid)) await sleep(50);
    if (processAlive(pid)) {
      ...
      if (stillOwned) {
        ...
        while (Date.now() < killDeadline && processAlive(pid)) await sleep(25);
      }
    }
    await rm(monitorPidPath(config), { force: true });
