...
async function assertSafeWorkspaceEnvironment(worktreePath) {
    ...
    if (!info.isFile() || info.isSymbolicLink())
        throw new Error("workspace .env must be a regular file");
    if (info.size > 1_000_000)
        throw new Error("workspace .env exceeds the 1000000-byte safety limit");
    const unsafe = unsafeHarnessEnvironmentNames(await readFile(target, "utf8"));
    if (unsafe.length)
        throw new Error(`workspace .env controls Harness bootstrap variables and is forbidden: ${unsafe.join(", ")}`);
}
...
async function runHarness(taskId, forcedModel) {
    ...
    if (selectedModel)
        env.DSH_MODEL = selectedModel;
    if (config.monitor.enabled) {
        if (!task.proxyToken)
            throw new Error("monitored Harness task has no proxy token");
...
function attempt(executor, model) {
    const value = { executor, startedAt: nowIso() };
    if (model)
        value.model = model;
    return value;
}
