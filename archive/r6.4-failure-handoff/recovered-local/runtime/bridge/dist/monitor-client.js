...
switch (command) {
    ...
    case "correct-cost": {
        ...
        if (!budgetGroupId || !rawCost || !reason)
            throw new Error("usage: monitor-client.js correct-cost <budgetGroupId> <correctedCostCny> <reason>");
        const correctedCostCny = Number(rawCost);
        if (!Number.isFinite(correctedCostCny) || correctedCostCny < 0)
            throw new Error("correctedCostCny must be a finite non-negative number");
    ...
    case "set-fx": {
        ...
        if (!rawRate || !asOf || !source)
            throw new Error("usage: monitor-client.js set-fx <usdToCnyRate|none> <asOf> <source>");
        const rate = rawRate.toLowerCase() === "none" ? null : Number(rawRate);
        if (rate !== null && (!Number.isFinite(rate) || rate <= 0))
            throw new Error("usdToCnyRate must be a positive number or none");
...
const exitCode = command === "status" && (!result || typeof result !== "object" || result.ok !== true) ? 1 : 0;
