...
args=sys.argv[1:]
if args in (["--version"],["-V"]): print("codex-fake 0.6.3"); raise SystemExit(0)
...
"$SOURCE_ROOT/scripts/install.sh" "${INSTALL_ARGS[@]}" --skip-self-tests
...
echo "[package 4/9] Schema v4 to v6 migration preserves custom values and installs adaptive policy"
...
set +e
FAKE_CODEX_FAIL_ADD=1 "$SOURCE_ROOT/scripts/install.sh" "${INSTALL_ARGS[@]}" --skip-self-tests >"$TEMP_ROOT/same-rollback.log" 2>&1
RC=$?
set -e
...
rm -rf "$INSTALL_ROOT"
set +e
FAKE_CODEX_FAIL_ADD=1 "$SOURCE_ROOT/scripts/install.sh" "${INSTALL_ARGS[@]}" --skip-self-tests >"$TEMP_ROOT/cross-rollback.log" 2>&1
RC=$?
set -e
...
echo "[package 7/9] Reinstall accepted R6.3 after rollback"
...
[[ ! -e "$SOURCE_ROOT/bridge/package-lock.json" ]]
find "$SOURCE_ROOT" -type l -print -quit | grep -q . && { echo "Package contains symlink" >&2; exit 1; } || true
python3 - "$SOURCE_ROOT" <<'PY'
import json,os,sys
root=sys.argv[1]
for rel in ['bridge/package.json','.codex-plugin/plugin.json','config/config.example.json','schemas/bridge-config.schema.json','schemas/task-envelope.schema.json','harness/minimal/profile/package.json','harness/minimal/MANAGED_MARKER.json']:
    json.load(open(os.path.join(root,rel),encoding='utf-8'))
print(json.dumps({
  "result":"PASS","version":"0.6.3","freshInstall":True,"installedAcceptance":True,
  "schema4To6Migration":True,"adaptiveSplitMemory":True,"tokenOnlyGates":True,
  "parallelMinimalHarness":True,"progressiveTools":True,"dsmlToolCallRecovery":True,
  "markdownShellToolCallRecovery":True,"nativeStructuredToolEvidence":True,"requiredChangeNoEffectIsolation":True,
