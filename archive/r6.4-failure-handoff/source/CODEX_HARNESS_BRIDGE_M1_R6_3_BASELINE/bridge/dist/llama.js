...
export async function stopManagedLlamaServer(config) {
    return await withNamedLock(config, "llama-managed-server", 30_000, async () => {
        ...
        if (processAlive(state.pid)) {
            try {
                process.kill(process.platform === "win32" ? state.pid : -state.pid, "SIGKILL");
            }
            catch { /* already gone */ }
        }
        await rm(managedStatePath(config), { force: true });
...
async function runCli(settings, task, prompt, maxOutput) {
    ...
    if (!containsPrompt)
        throw new LlamaExecutionError("process", "llama.cpp cliArgs must include {{PROMPT}} or {{PROMPT_FILE}}");
    ...
    if (result.stderr)
        await writeFile(task.stderrPath, `[llama-cli ${commandDisplay(settings.cliBinary, args)}]\n${result.stderr}\n`, { encoding: "utf8", mode: 0o600, flag: "a" });
    if (result.timedOut)
