...
export async function runProcess(command, args, options = {}) {
    ...
    return await new Promise((resolve, reject) => {
        ...
        child.once("exit", () => {
            // When a controlled command leader exits, no descendant is allowed to
            // outlive it.  Killing the detached process group here (rather than only
            // on `close`) is important: a stray descendant can otherwise retain the
            // stdout/stderr pipe and prevent Node from ever emitting `close`.
            if (useProcessGroup && child.pid) {
                try {
        ...
        child.once("close", (code, signal) => {
            ...
            resolve({ code, stdout, stderr, timedOut, signal, stdoutTruncated, stderrTruncated });
        });
        ...
        if (options.timeoutMs && options.timeoutMs > 0) {
            timer = setTimeout(() => {
                ...
                escalationTimer = setTimeout(() => signalProcess("SIGKILL"), 5_000);
