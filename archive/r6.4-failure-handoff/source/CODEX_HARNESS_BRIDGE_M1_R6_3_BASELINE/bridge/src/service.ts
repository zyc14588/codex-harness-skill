  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      while (Date.now() < deadline) {
        ...
        if (!processAlive(workerPid)) break;
      ...
      if (child.pid && processAlive(child.pid)) {
        try { process.kill(-child.pid, "SIGTERM"); } catch { try { process.kill(child.pid, "SIGTERM"); } catch { /* already gone */ } }
      }
...
function validatedHarnessUpstreamBaseUrl(config: BridgeConfig): string {
  ...
  let value: URL;
  try { value = new URL(raw); } catch { throw new Error("DEEPSEEK_BASE_URL must be a valid absolute URL"); }
  if (value.protocol !== "http:" && value.protocol !== "https:") throw new Error("DEEPSEEK_BASE_URL must use http or https");
  if (value.username || value.password) throw new Error("DEEPSEEK_BASE_URL must not embed credentials; use DEEPSEEK_API_KEY");
...
async function normalizedLeaf(
  ...
  const taskFamily = boundedText(
    input.taskFamily?.trim() || defaultTaskFamily(mode, harnessWritePaths, executor, requestedModel),
    `leaf ${id} taskFamily`,
    500,
  );
  ...
  if (requestedModel) leaf.model = requestedModel;
  return leaf;
}
...
export async function startTask(input: StartTaskInput): Promise<unknown> {
  ...
  return await withMutationLock(config, async () => {
    ...
    if (unsafeEnvNames.length) throw new Error(`tracked .env controls Harness bootstrap variables and is forbidden: ${unsafeEnvNames.join(", ")}`);
