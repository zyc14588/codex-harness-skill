...
const DEFAULT_PASS_ENVIRONMENT = [
  ...
  "DEEPSEEK_API_KEY", "DEEPSEEK_BASE_URL", "DSH_MODEL", "LLAMA_CPP_API_KEY",
...
export function normalizeLlamaConfig(value: unknown, fallback?: LlamaCppConfig): LlamaCppConfig {
  ...
  const config: LlamaCppConfig = {
    ...
    apiKeyEnv: stringValue(raw.apiKeyEnv, defaults.apiKeyEnv, "llamaCpp.apiKeyEnv"),
    model: stringValue(raw.model, defaults.model, "llamaCpp.model"),
    serverBinary: stringValue(raw.serverBinary, defaults.serverBinary, "llamaCpp.serverBinary"),
    serverArgs: stringArray(raw.serverArgs, defaults.serverArgs, "llamaCpp.serverArgs"),
    ...
    cliBinary: stringValue(raw.cliBinary, defaults.cliBinary, "llamaCpp.cliBinary"),
    cliArgs: stringArray(raw.cliArgs, defaults.cliArgs, "llamaCpp.cliArgs"),
...
export async function loadConfig(): Promise<BridgeConfig> {
  ...
  if (![1, 2, 3, 4, 5, 6].includes(Number(parsed.schemaVersion))) throw new Error(`unsupported config schema at ${configPath}`);
  ...
  const config: BridgeConfig = {
    ...
    stateRoot: path.resolve(expandHome(stringValue(parsed.stateRoot, "~/.local/state/codex-harness-bridge", "stateRoot"))),
