...
const temp = await mkdtemp(path.join(os.tmpdir(), "codex-harness-m1-r6-3-acceptance-"));
...
try {
  ...
  assert.equal(textualTerminal.minimalMutationPolicyVersion, "minimal-flash-required-v1", JSON.stringify(textualTerminal));
  ...
  assert.equal(requiredViolationTerminal.minimalMutationPolicyVersion, "minimal-flash-required-v1", JSON.stringify(requiredViolationTerminal));
